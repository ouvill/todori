...
- **2026-07-07ドッグフーディング第2回実施。** フィードバック11件のうち、task-44/45/46は完了、task-47は予定としてタスク化。task-47以外の残件は後続バックログ補充で再確認する。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-47: Todayスマートリスト化+リスト一覧Todayリンク | Todayを全リスト横断（アーカイブ済みリスト除外）のスマートリストへ移行し、リスト一覧にTodayリンクを追加する。対象は期日今日+期日超過、期日なしは対象外、TodayでのAdd taskは既定Inboxに今日期日で作成する | M4系 | 出典: ドッグフーディング2026-07-07#2 |
