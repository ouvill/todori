...
| 5 | DBスキーママイグレーション機構の整備 | `core/storage` にスキーマバージョニングとマイグレーション実行機構を整備し、誤鍵/旧版DBのテストを追加する | M4前提（core/storage） | 出典: 2026-07-06人間裁定（要人間判断→確定）。設定値の永続化（#9）を含む以後のスキーマ変更をすべてマイグレーション経由にするための前提 |
