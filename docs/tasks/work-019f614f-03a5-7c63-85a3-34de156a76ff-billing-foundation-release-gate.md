...
初回対象はiOSの個人向けProだけとする。RevenueCatをprovider adapterの初期実装とし、月額・年額商品を単一の`pro` entitlementへ束ねる。最終価格は人間承認まで未決とし、価格をコードやpublic文書へ固定しない。
...
| price | 人間承認待ち。コードへ固定せずStore metadataから表示 |
...
- serverは環境別のRevenueCat project ID、read-only secret key、allowed app ID、webhook Authorization値、HMAC secretと、`SANDBOX / PRODUCTION`の実行環境を環境変数から読む。clientもbuild environment別のpublic SDK keyをsecret store / CI設定から受け取る。実値はcommitせず、同じproject / appをsandboxとproductionで兼用しない。
...
- HMAC secretはRevenueCat側rotateで旧secretが即時無効になるため、server secret更新→server deploy完了→RevenueCat rotate→新secretで再deployを同一maintenance windowで行い、test webhook成功まで監視する。通常rotationでdual-secret期間を仮定しない。
...
- 本work itemのproduct、trial、grace、restore、launch offer、provider判断は2026-07-16に承認済みである。価格だけは未承認であり、App Store product作成とApple sandbox E2Eの停止条件とする。
