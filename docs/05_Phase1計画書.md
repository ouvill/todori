...
Phase 1の先行プラットフォームはiOSとする（`docs/01_企画書.md` §8、`docs/06_事業・法務方針.md` §6）。開発ホストがLinuxであるため、日常のdogfoodingと自動テスト用にLinuxデスクトップビルドを維持し、iOS実機/Simulator確認はmacOSランナーまたは実機環境で別途実施する。
...
| 通知 | ローカル通知（F-24）、複数通知時刻・スヌーズの最小版（F-25）、iOS通知を先行。デスクトップ通知（F-26）はLinux dogfooding用の動作確認に留める | E2EE設計上ローカル通知が正となるため |
...
| M1-04 | DK生成、OSキーチェーン保存、SQLCipher鍵導出の抽象を定義する | M1-02 | LinuxテストダブルでDK生成からDB openまでのテストが通ること |
...
| M5-02 | Linux dogfoodingビルドを配布可能にする | M4 | Linux desktopで主要操作が通り、既知差分がリリースノートに記録されていること |
...
| リスク | 影響 | 対策または判断 |
|---|---|---|
| SQLCipherのiOS/Androidビルド差分 | Phase 1 iOSリリース不能 | task-02のホスト成功を前提に、M1でiOS/macOSランナー検証を最優先にする |
