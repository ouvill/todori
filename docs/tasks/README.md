...
| task-24 | 実装完了（親検証待ち） | [task-24-fractional-index.md](./task-24-fractional-index.md) | fractional index生成の本実装と、タスク一覧の同一階層内手動並び替えUIを追加する | task-18〜23の成果物に依存。M3-05のうちUndoと条件ソートUIは後続タスクへ分離 |
