...
task-12「OSS公開前監査」では、Taskveilをpublic repository化する前に確認すべき秘密情報、公開不適切情報、OSS基本文書、CI/Actions安全性を棚卸しした。task-13「public/privateドキュメント分割方針の策定」では、ユーザーが選択した「public repoを主、private repoを非公開資料側」とする方針に基づき、文書ごとのpublic/private分類、要約ルール、後続タスク候補を整理した。

このタスクは、task-13の移行計画を実際の作業指示に落とし込み、public repoとして残す `taskveil/` と、private側の sibling repo を分けて運用できる初期状態を作るためのものである。

CodexなどのAIコーディングエージェントは、同じワークスペース配下に sibling repo として配置された複数のGit repositoryを同時に参照できる。そのため、推奨構成は以下とする。

```text
taskveil-root/
├── taskveil/                 # public repo: コード・公開ドキュメントの正本
└── taskveil-private/         # private repo: 事業/法務/監査/内部資料
```

private repoを `taskveil/` の内側へネストしたり、public repoからprivate repoをsubmodule参照したりすると、private repoの存在やURL、内部運用の痕跡がpublic側に残りやすい。このタスクでは sibling repo 運用を前提にする。

ユーザー判断として、private repo名は `taskveil-private` とする。task-12/task-13時点で実シークレットや証明書のコミットは確認されていないため、このタスクではGit履歴rewriteは行わず、現在のファイル配置と公開版要約を整える方針とする。履歴上の課金・法務詳細を完全に消す必要が後から生じた場合のみ、別タスクで履歴分離を扱う。
...
Taskveilをpublic/privateの2リポジトリで運用するため、以下を実施する。
...
1. **作業ツリー確認**: `git -C taskveil status --short` でpublic repoの未コミット変更を確認する。private repoが既に存在する場合は `git -C taskveil-private status --short` も確認する。
2. **private repo配置確認**: `taskveil-root/taskveil-private/` が存在するか確認する。存在しない場合は、作成する前に完了報告へ「人間がGitHub private repoを作成またはローカル初期化する必要がある」と記録する。実際に `git init` するかどうかは人間判断とする。
...
7. **複数repo運用手順の記録**: 完了報告に、Codexで sibling repo を同時に扱う際の基本コマンド（例: `git -C taskveil status`, `git -C taskveil-private status`）と注意点を記録する。
8. **public混入チェック**: public repo側にprivate詳細が残っていないか、少なくとも `docs/04` / `docs/06` / `README.md` / 新規要約文書を目視確認し、必要な検索を実行する。
9. **検証**: ドキュメントのみの変更として、最低限 `git -C taskveil diff --check` を実行する。private repoを作成または変更した場合は、private側でも `git -C taskveil-private status --short` を確認する。
...
1. `git -C taskveil status --short` を実行し、public repoの状態を確認する。
2. `taskveil-private/` の有無を確認する。存在する場合は `git -C taskveil-private status --short` を実行する。存在しない場合は、作成要否を完了報告の人間判断事項に残す。
...
8. `rg -n "MRR|ARR|ARPU|転換率|損益分岐|個人事業|暗号輸出|商標|ドメイン|RevenueCat|Stripe" taskveil/README.md taskveil/docs/billing_overview.md taskveil/docs/legal_overview.md` などで、要約文書に詳細情報が混入していないか確認する。
9. `git -C taskveil diff --check` を実行する。
...
- [x] `git -C taskveil diff --check` が成功している。
...
- private repoの名前は `taskveil-private` とする。GitHub org/user、権限、branch protection、バックアップ方針は人間判断である。
...
- Codexで複数repoを扱う運用手順
- public混入チェックの結果
- 検証結果（少なくとも `git -C taskveil diff --check`）
- 未解決事項・要人間判断
...
初回完了時点では `taskveil-root/taskveil-private/` が存在しなかったためprivate側の実ファイル作成は行わなかった。その後、ユーザーが `taskveil-private/` を作成したため、追加作業として以下を作成した。
...
- `taskveil-private/business/billing-design.md`
...
- Taskveilというproject identityを公開文書で一貫して使う方針。
...
- `taskveil-private/legal/business-legal-policy.md`
...
- public repo確認: `git -C taskveil status --short`
- private repo確認: `git -C taskveil-private status --short`
- private repoは `taskveil/` の内側に置かない。
- public repoからprivate repoをsubmodule/subtree参照しない。
...
- `git -C taskveil-private diff --check`: 成功。

### 未解決事項・要人間判断

- `taskveil-private` repositoryのGitHub上のアクセス権限、branch protection、バックアップ方針は人間作業である。
- `docs/04_課金設計書.md` と `docs/06_事業・法務方針.md` の詳細版ファイルは、追加作業でpublic repoから削除し、`taskveil-private/` へ退避した。
