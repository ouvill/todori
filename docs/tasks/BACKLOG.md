...
| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | wont_do / 再オープンのUIステータス遷移 | `done` / `wont_do` / 再オープンの各ステータス遷移をUIから実行できるようにする。禁止遷移は表示上選べないこと | M3-04 | 出典: M3-04（現行UIはラベル表示のみ。domain/Rust側にはwont_doが存在。2026-07-06親棚卸しで確認） |
| 2 | DBスキーママイグレーション機構の整備（task-36） | `core/storage` にスキーマバージョニングとマイグレーション実行機構を整備し、誤鍵/旧版DBのテストを追加する | M4前提（core/storage） | 出典: 2026-07-06人間裁定（要人間判断→確定）。設定値の永続化（#10）を含む以後のスキーマ変更をすべてマイグレーション経由にするための前提。task-37アーカイブの前提（`lists.archived_at` 追加が最初のマイグレーション題材） |
| 3 | リストのアーカイブ/解除（task-37） | リストのアーカイブ（`lists.archived_at`）/アーカイブ解除を実装する。アーカイブ済みリストは通常一覧から分離表示（アーカイブ済みセクション等）し、データ・完了履歴は完全保全する（F-09改訂準拠） | M3-01相当 | 出典: 2026-07-07人間裁定（削除モデル）。docs/02 F-09改訂準拠。task-36（DBスキーママイグレーション機構）完了後に着手 |
| 4 | ゴミ箱廃止と恒久削除への移行（task-38） | trash UI/route/API（task-23実装分）を撤去し、タスク・リストの削除を物理削除＋不可逆警告の追加確認へ移行する。削除Undoは廃止する（完了/編集Undoは維持）。`tasks.deleted_at`カラムおよびundo履歴の削除系エントリの整理はマイグレーション経由で行う | M3-01 / M3-05相当 | 出典: 2026-07-07人間裁定（削除モデル）。docs/05 ADR-009 / docs/02 F-07改訂準拠。task-37（リストアーカイブ）の直後に着手 |
| 5 | タスク行のdot/チェック整列修正 | priority dotとチェックをタイトル1行目とセンター整列させる（行全体センターではない） | 軽量レーン | 出典: 親レビュー2026-07-06（`docs/design/ui-spec.md` セクション5「既知の逸脱」参照） |
...
| 10 | 設定値の永続化機構とF-01 UIモード設定の保存口 | 設定値を永続化する仕組みを用意し、F-01のUIモード選択の保存口を実装する | M4 | 出典: Phase1計画書§1（F-01「設定値の保存口のみ用意」）。通知設定・UIモードの前提。2026-07-06親棚卸しで確認 |
| 11 | アクセシビリティ検証パス | Dynamic Type / スクリーンリーダーラベル / コントラストの確認項目を通す | M4-03 | |
| 12 | 性能検証 | 1万件データで起動2秒以内・主要操作60fps・オフライン動作を計測し、結果を記録する | M4-04 / F-50〜F-52 | |
| 13 | 日付・時刻表記のロケール準拠リファクタ | 固定パターン `DateFormat('EEE, MMM d')` 等をskeleton API（`yMMMEd`等）へ置換し、ホストのロケール設定に従う | M4系 | 出典: 2026-07-06人間指示（`docs/design/ui-spec.md`参照）。ja表示「月, 7月 6」の不自然さもこれで解消 |
| 14 | オンボーディング/初回起動体験 | 範囲設計のplannerタスクから開始する。DK復旧不可の注意表示（計画書§5リスク表）を含む。マスコットの利用はvisual-direction.mdの方針に従う | M4系 | 出典: 2026-07-06人間裁定（要人間判断→確定） |
| 15 | iOSリリースビルド/署名/ストア提出準備 | macOS環境でReleaseビルドが成功し、ストア提出前のコンプライアンス確認項目を整理する | M5-01 | |
| 16 | Phase 1リリース前にthemeModeをライト固定 | ダークモード正式対応まではアプリの`themeMode`をlight固定にする。dark系トークン・コードは残置し、priority dot固定hexのコントラスト検証等の磨き込みはダークモード対応再開時（裁定により直近スコープ外）に行う | M5系 | 出典: 2026-07-06人間裁定 |
| 17 | macOS dogfoodingビルド配布 | macOS desktopで主要操作が通り、既知差分をリリースノートに記録する | M5-02 | |
| 18 | クラッシュレポート方針の確定 | F-53オプトイン文言・PII除去対象・実送信するかの判断を記録する | M5-03 | |
| 19 | SQLCipherクロスビルドのiOS/Android CI検証 | iOS/AndroidのSQLCipherビルド差分をCIで継続検証する | Phase1計画書§6 | |
