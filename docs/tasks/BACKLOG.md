...
| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-42: 詳細画面インライン編集 | タスク詳細の右上編集ボタンと一括編集ダイアログを撤去し、タイトル/ノート/期日/優先度を詳細画面上で直接編集できるようにする | 軽量レーン | 出典: ドッグフーディング2026-07-07 |
| 2 | Design Lab準拠のビジュアル統一と遷移動線（task-43予定） | 本番画面をDesign Lab方向へ寄せ、画面遷移動線を整理する。既存のdot整列修正とLucide統一もこの作業へ吸収する | 軽量レーン | 出典: ドッグフーディング2026-07-07。吸収: 親レビュー2026-07-06のdot整列、2026-07-06人間裁定のLucide統一 |
