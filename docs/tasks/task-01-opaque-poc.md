...
OPAQUEの最大の特徴は、ログイン成功時にクライアント側だけが `exportKey` という秘密値を導出できる点である。Todoriはこの `exportKey` を直接Master Keyとして使うのではなく、HKDFで`KEK_pw`（鍵暗号鍵）を導出し、`KEK_pw` でMaster Key (MK) をラップ（`wrap(MK, KEK_pw)`）してサーバーに保存する。こうすることで、パスワード変更時もMK自体を変えずに済み、タスクデータの再暗号化が不要になる（詳細設計は `docs/03_技術仕様書.md` §4.2〜4.5参照）。
...
2. **CipherSuite定義**: `core/crypto/src/opaque.rs`（新規ファイル）に、`opaque-ke::CipherSuite` を実装する型（例: `pub struct TodoriCipherSuite;`）を定義する。以下の選定をコード内ドキュメントコメントで明記すること。
...
6. **KEK_pw導出とMKラップのテスト**: ログインで得た `exportKey` を入力鍵材料 (`ikm`) として、既存の `todori_crypto::kdf::derive_key(ikm, info)` を用いて `KEK_pw`（32byte）を導出する。`info` にはバージョン付きの文脈文字列（例: `b"todori/kek-pw/v1"`）を用いること。ダミーのMaster Key（32byteの乱数、`OsRng`等で生成）を、既存の `todori_crypto::aead::encrypt` / `decrypt` で `KEK_pw` を鍵として wrap / unwrap し、unwrap結果が元のMKと一致することを検証する。
...
6. `cargo test -p todori-crypto` でこのcrate単体のテストを繰り返し実行しながら実装する。
...
- [ ] `cargo test -p todori-crypto opaque::` で本タスクの新規テストがすべて実行され成功する
...
- `core/crypto/src/opaque.rs` を追加し、`core/crypto/src/lib.rs` から `pub mod opaque;` と `TodoriCipherSuite` のre-exportを追加した。
...
| 対象 | サイズ |
|---|---:|
| `ServerSetup<TodoriCipherSuite>` serialize結果 | 128 bytes |
| `ServerLogin<TodoriCipherSuite>` state serialize結果 | 192 bytes |
...
- `cargo test -p todori-crypto opaque::` 成功。
