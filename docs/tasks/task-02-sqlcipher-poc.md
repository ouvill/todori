...
Taskveilのローカルストレージは「素のSQLite + SQLCipher」を採用する設計になっている（`docs/03_技術仕様書.md` §5）。SQLCipherの鍵は常にDevice Key (DK) からHKDFで導出した値を使う。しかし、Rustから SQLCipher を利用する `rusqlite` の `bundled-sqlcipher` 系featureが実際に各プラットフォーム（特にAndroid/iOS）でビルドできるかどうかは、`docs/03_技術仕様書.md` §12「未決事項リスト」の筆頭に挙げられている未検証事項である。
...
`taskveil-storage` crateに `rusqlite`（SQLCipher有効化feature）を導入し、暗号化DBに対する読み書き・誤り鍵での失敗・FTS5全文検索が動作することをテストで実証する。あわせて `TaskRepository` の最小実装（insert/get）をSQLite上に構築する。
...
6. **Androidクロスビルド調査**: `cargo-ndk` の導入（`cargo install cargo-ndk`）を試み、Android NDKが利用可能な場合は `cargo ndk -t arm64-v8a -o <out-dir> build -p taskveil-storage` を実行してビルド可否を確認する。NDKが環境に存在しない、あるいはツールチェーンの制約でビルドが完了しない場合は、**その旨と必要な準備手順（NDKバージョン、環境変数、`.cargo/config.toml`設定例など）を調査し文書化するだけでよい**。この調査自体が失敗してもタスク全体の受け入れ基準には影響しない（6章参照）。

### やらないこと

- iOS/macOS向けのビルド検証（macOSホストが必要なため、必要な手順の文書化のみ行い実機検証はしない）。
- libSQL/Tursoは採用しないこととなった（`docs/05_設計判断記録.md` ADR-002参照）。
- スキーママイグレーション機構の実装。
...
3. `cargo build -p taskveil-storage` を実行し、ビルドが通ることを確認する（初回はOpenSSLのビルドがあるため数分かかる可能性がある。所要時間を記録する）。ビルドに失敗した場合はfeatureを `bundled-sqlcipher` に変更して再試行し、結果を報告に記載する。
4. `core/storage/src/schema.sql` を作成する。
5. `core/storage/src/lib.rs` に `open_encrypted` と `SqliteTaskRepository` を実装する。
6. テストを実装し、`cargo test -p taskveil-storage` で繰り返し検証する。
...
- [ ] `cargo test -p taskveil-storage` で上記5.の(1)〜(5)のテストがすべて成功する
- [ ] Androidクロスビルドの調査結果（成功/失敗いずれでも可）が完了報告に記載されている

## 7. 制約・注意事項

- SQLCipherの鍵は本PoCでは固定のテスト用32byte値（例えば `[0x11; 32]` 等）で構わない。DKからのHKDF導出は `taskveil-crypto::kdf::derive_key` を用いる想定だが、`core/storage` から `core/crypto` への依存追加は本タスクのスコープ外とする（テストでは鍵をその場で用意してよい）。
...
- ビルド後のバイナリ/rlibサイズの増分（`cargo build -p taskveil-storage` 前後での比較）
...
- `rusqlite 0.40.1` を `bundled-sqlcipher-vendored-openssl` feature付きで採用した。
- `core/storage/src/schema.sql` を追加し、`tasks` tableとPoC用 `tasks_fts` FTS5 tableを定義した。
- `open_encrypted(path, key)` を実装し、`PRAGMA key = "x'<hex>'"` 形式でSQLCipher raw keyを適用するようにした。
- `SqliteTaskRepository` を追加し、`TaskRepository::insert` / `get` をSQLite上に実装した。`update` はPoC範囲に従い `StorageError::NotImplemented` のまま。

### 採用依存

| crate | version / feature |
|---|---|
| `rusqlite` | `0.40.1`, `bundled-sqlcipher-vendored-openssl` |
...
`bundled-sqlcipher-vendored-openssl` はホスト環境でビルド・テストが通ったため、システムOpenSSL依存を避けられる第一候補として採用した。

### ビルド・サイズ

- 初回 `cargo test -p taskveil-storage` はOpenSSL/SQLCipher vendoredビルド込みで約52秒。
- 比較前のrlibサイズは取得していなかったため厳密な増分は未測定。
- 現在の代表値:
  - host debug rlib: 約1.0 MiB
  - Android `aarch64-linux-android` debug rlib: 約1.4 MiB
- vendored OpenSSL/SQLCipherはCIキャッシュなしだと初回ビルド時間が重くなるため、CIではCargo registry/git cacheとtarget cacheの導入が望ましい。
...
cargo ndk -t arm64-v8a -o /private/tmp/taskveil-ndk build -p taskveil-storage
```

結果:

- `cargo-ndk 4.1.2` の導入に成功した。
- 最初は `aarch64-linux-android` Rust target未導入で失敗したため、`rustup target add aarch64-linux-android` を実施した。
- 再実行では `taskveil-storage` のAndroid向けコンパイル自体は完了した。
- 最後に `cargo-ndk` が出力先へコピーする段階で、`taskveil-storage` が通常のRust libraryで `cdylib` を出さないため `No usable artifacts produced by cargo` で停止した。

所見:

- SQLCipher/OpenSSL vendored構成はAndroid target向けにもコンパイル可能と見てよい。
- Flutterアプリへ組み込むには、`taskveil-storage` 単体ではなく `taskveil-app-bridge` のような `cdylib` / `staticlib` crate経由でリンクする設計にする必要がある。

### docs/03 §12への回答案

`rusqlite` + `bundled-sqlcipher-vendored-openssl` は、ホスト環境で暗号化DB・誤鍵失敗・平文SQLite不可・FTS5・repository roundtripが通り、Android targetでも少なくともRust libraryとしてはコンパイルできた。したがって、SQLCipher統合方式の第一候補はこの構成で進める。未決事項として残すべきなのは「iOS/AndroidのFlutterビルドパイプラインへ、最終的なcdylib/staticlibとして組み込めるか」の検証である。

### 検証

- `cargo test -p taskveil-storage` 成功。
- `cargo test --workspace` 成功。
- `cargo clippy --workspace -- -D warnings` 成功。

### 未解決事項

- iOS/macOS実機またはCIランナーでのSQLCipherリンク検証は未実施。
- Androidの最終成果物コピーは `taskveil-storage` が `cdylib` ではないため未完了。Phase 1では `taskveil-app-bridge` 側にstorageをリンクして検証する必要がある。
