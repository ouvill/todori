...
| task-02 SQLCipher PoC | M1の前提。`core/storage` の暗号化DB、FTS5、repository実装に直結する | host / iOS Simulator core test、iOS実機target link、Android Rust FFI / arm64 release APK buildまで成功。非docs変更ごとのiOS no-codesign release buildとAndroid release APK buildをCIへ導入済みで、iOS / Android実機継続検証が残る |
...
- SQLCipherとFRBのhost / iOS / Android基礎検証は成立し、iOS no-codesign release build CI、Android release APK CI、Android Keystore実装まで完了した。残る継続検証はiOS / Android実機、Android接続実機でのKeystore / DB reopenであり、一般リリースはさらに課金release gateへ依存する。
