...
- frontend-neutralな`ClientProfile`を`taskveil-client`へ実装し、profile openからCRUD、account、syncまでの唯一の高水準入口にする。
...
- app manifestのworkspace内依存を`taskveil-client`だけにし、boundary checkからlegacy例外/count baselineを削除する。
...
- [x] `app/rust`非生成sourceに`taskveil_(crypto|domain|storage|sync)`、`open_encrypted`、`Sqlite*`、`AccountClient`、`LocalSyncStore`、`LocalMutationContext`が0件。
- [x] `app/rust/Cargo.toml`のTaskveil workspace内依存が`taskveil-client`だけで、`support.rs` / `sync_store.rs`が削除される。
...
- [x] CLI/MCPが`taskveil-client`以外の下位crateへ依存せずClientProfile APIをcompile時に参照する。
...
- `taskveil-client`はFlutter、Dart、FRB、clap、MCP transportへ依存しない。
...
- Bridge: handwritten Rustを`api.rs` 606行、`profile_handle.rs`、`lib.rs`の薄いadapterへ縮小した（移設前`api.rs` 1,148 + `support.rs` 1,243 + `sync_store.rs` 2 = 2,393行）。`support.rs` / `sync_store.rs`を削除し、appのTaskveil workspace内依存を`taskveil-client`だけにした。既存同期FRB署名用Tokio executorは`profile_handle.rs`だけに限定する。39公開関数の正規化signature hashは移設前後で一致し、8 DTOとDart call surfaceを維持した。
- Public API: 通常の`taskveil-client`公開面を`ClientProfile`、frontend-neutral model/domain view、`ClientError`へ限定した。低水準Client/local crypto/SQLite sync storeは`test-support` featureへ隔離し、server integration testだけが明示的に有効化する。root import不可をcompile-fail doctestで固定した。
...
> 追補（task-94）: 本taskで導入した`ClientProfile` / `ProfileConfig`は、責務を明瞭にするため`TaskveilClient` / `LocalProfileConfig`へbreaking renameした。旧名aliasは残していない。
