...
5. リスト削除時の配下タスクの扱い: **2026-07-07人間裁定により以下へ確定した（旧暫定解「既定インボックスへの全件再割り当て」は撤回する）**。
   - 削除対象リストに属する**アクティブタスク（`deleted_at` 未設定）はすべてゴミ箱へ送る**。F-07（ゴミ箱・Undo）が既に定める論理削除の仕組み（`deleted_at` 設定）にそのまま乗せるカスケード論理削除であり、タスク自体は削除しない。
   - 削除対象リストに属していて**既にゴミ箱内（`deleted_at` 設定済み）のタスクはそのまま据え置く**（何も変更しない）。
   - ゴミ箱からの**復元時、元リストが削除済みの場合は既定インボックスへ復元する**（restore経路で `list_id` を既定インボックス（本節3.の定義に従う）へ再マッピングする）。
   - 削除確認ダイアログには**配下のアクティブタスク件数を明示する**（例:「このリストと3件のタスクをゴミ箱へ移動します」相当。文言はl10nで用意し `docs/design/ui-spec.md` のDialog規範に従う）。
   - **裁定根拠**: リストは「大きなタスク（プロジェクト的）」とも「継続する生活領域（エリア的）」とも解釈できるが、いずれのモデルで捉えてもリスト削除に伴う配下アクティブタスクのゴミ箱送りは驚きを生まず、誤削除に対する保険は既存のゴミ箱の仕組みがそのまま果たす。インボックス直行案は「リストを削除したのにインボックスのタスク件数が膨らむ」という違和感を生むため不採用とした。
...
- 削除時、配下の**アクティブタスクはゴミ箱へ送られ**（既にゴミ箱内のタスクはそのまま）、消失しない。ゴミ箱からの復元時、元リストが削除済みなら既定インボックスへ`list_id`が再マッピングされて復元される。
...
2. **`core/storage`**:
   - `ListRepository` trait に `delete` を追加する（シグネチャ例: `fn delete(&mut self, list_id: Uuid, now_ms: i64) -> Result<(), StorageError>`。実装は同一SQLite接続内のトランザクションで (a) `list_id` に属する**アクティブタスク（`deleted_at IS NULL`）のみ**を論理削除する（`deleted_at` を `now_ms` に設定。既存のF-07ゴミ箱送り実装と同一のフィールド更新に揃えること）、(b) 既にゴミ箱内（`deleted_at IS NOT NULL`）のタスクには触れない、(c) `lists` テーブルから対象行を削除する。対象リストが存在しない場合は `StorageError::NotFound` を返す。
   - タスクの復元系メソッド（`TaskRepository` の restore相当）に、復元対象タスクの `list_id` が指す `lists` 行が存在しない場合は既定インボックスの `list_id` へ差し替えて復元する処理を追加する（restore経路の `list_id` 再マッピング。既定インボックスの特定は「既定インボックス保護について」3節の定義（`sort_order` 最小）に揃える）。
   - 統合テストを追加する: (a) 削除対象リストのアクティブタスクがゴミ箱送り（`deleted_at` 設定）されること、(b) 削除前から既にゴミ箱内だったタスクの `deleted_at` が変化しないこと、(c) 削除後に対象リストが `list_all()` に現れないこと、(d) 存在しないリストIDの削除が `NotFound` を返すこと、(e) 削除済みリストに属していたゴミ箱内タスクを復元すると既定インボックスへ `list_id` が再マッピングされること。
3. **`app/rust/src/api.rs`**:
   ...
   - `delete_list(list_id: String) -> Result<(), String>` を追加する。実装は (a) `list_all()` で全リストを取得し先頭（`sort_order` 最小）を既定インボックスとする、(b) `ensure_list_deletable` で対象が既定インボックスでないか検証する、(c) `ListRepository::delete` を呼び、配下のアクティブタスクをゴミ箱送りにしたうえで対象リストを削除する。
   - 削除確認ダイアログに配下アクティブタスク件数を表示するため、既存の `list_active_by_list` 相当の仕組みを流用するか軽量な件数取得APIを追加する（実装時に判断してよい）。
...
4. **Dart（bridge / provider）**:
   - `BridgeService`（`app/lib/src/core/bridge_service.dart`）に `renameList` / `deleteList` を追加し、`FrbBridgeService` 実装と `FakeBridgeService`（`app/test/support/fake_bridge_service.dart`）の両方に実装する。`FakeBridgeService` の削除実装も、既定インボックス保護・配下アクティブタスクのゴミ箱送り・復元時の `list_id` 再マッピングのロジックを模倣すること（本番ロジックと矛盾するテストにしない）。
   - `ListsNotifier`（`app/lib/src/core/providers.dart`）に `renameList` / `deleteList` メソッドを追加する。削除後は `listsProvider` の invalidate に加え、削除対象リストの `tasksProvider`（配下タスクがゴミ箱送りされ空になるため）、`trashedTasksProvider`（配下タスクが増えるため）も invalidate する。復元操作の呼び出し元では、復元後に既定インボックスの `tasksProvider` も invalidate すること（remap先へ反映するため）。
5. **UI（Lists画面）**:
   ...
   - 削除: `showAppConfirmDialog` を用いた確認ダイアログを表示する。**配下のアクティブタスク件数を取得し、ダイアログ本文に明示する**（例:「このリストと3件のタスクをゴミ箱へ移動します」相当。0件の場合の文言もl10nで用意する）。確認ボタンの配色は `docs/design/ui-spec.md` の destructive coral (`#E8755A`) 規則に従う。
   - 既定インボックス行（`lists.first`）には削除メニュー項目を表示しない、または無効化する。名称変更は既定インボックスでも可能なままにする。
6. **l10n / test**:
   - 名称変更・削除（配下アクティブタスク件数を含む確認文言）に関するUI文字列をen/ja両方のARBへ追加し `flutter gen-l10n` を実行する（`app/lib/src/generated/l10n/` は生成差分のみ）。
   - widget testを追加する: (a) 名称変更がLists画面・FakeBridgeServiceへ反映される、(b) 削除確認ダイアログに配下アクティブタスク件数が表示される、(c) 削除確認後にリストが消え配下のアクティブタスクがゴミ箱へ移動する、(d) 削除前から既にゴミ箱内だったタスクは変化しない、(e) 削除済みリストに属していたゴミ箱内タスクを復元すると既定インボックスへ再マッピングされる（restore remap確認。widget testまたは統合テストのいずれかで可）、(f) 既定インボックスに削除導線が出ない/選べない、(g) 長いリスト名でも改名ダイアログ・確認ダイアログが破綻しない。
7. **visual QA**:
...
- [ ] UI（Lists画面）からリストを削除すると、当該リストがDBから消え、配下の**アクティブタスクがゴミ箱へ送られる**（`deleted_at` 設定）ことがstorage統合テストで確認できる。削除前から既にゴミ箱内だったタスクの `deleted_at` が変化しないこともあわせて確認できる。
- [ ] ゴミ箱からタスクを復元する際、元リストが削除済みの場合は既定インボックスへ `list_id` が再マッピングされて復元されることが、統合テストまたはwidget testで確認できる。
- [ ] 削除確認ダイアログに配下のアクティブタスク件数が表示されることがwidget testで確認できる。
...
- リスト削除はタスクのデータ消失を伴ってはならない（2026-07-07人間裁定: 配下のアクティブタスクはゴミ箱へ送る論理削除とし、既にゴミ箱内のタスクはそのまま。復元時に元リストが削除済みなら既定インボックスへ `list_id` を再マッピングする。詳細は本指示書「既定インボックス保護について」5節）。
...
- 作業日、読んだファイル
- F-09から読み取った削除セマンティクス・既定インボックス保護の判断根拠、および2026-07-07人間裁定（配下アクティブタスクのゴミ箱送り・復元時の既定インボックスremap）と実装が一致しているか（本指示書「既定インボックス保護について」1〜5節参照。逸脱した場合はその内容）
- `core/domain` に追加した関数・`DomainError` variant・単体テスト結果
- `core/storage` の `ListRepository::delete` 実装内容（トランザクション境界、ゴミ箱送り対象の範囲）と、復元系メソッドに追加した既定インボックスremap実装、それぞれの統合テスト結果
...
- 品質ゲートの実行結果一覧
- 未解決事項（既定インボックス改名可否、既定インボックス自動プロビジョニングの要否、restore remap実装箇所の妥当性等を含む）
