...
- **2026-07-07ドッグフーディング実施。** フィードバック5件をtask-40/41/42（完了）とtask-43/44（後続）へタスク化。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-43: Design Lab準拠のタスク一覧ビジュアル整合 | 本番Today/タスク一覧をDesign Labの構造へ寄せる。Tasks単一パネル化、priority dotのメタデータ行移動、通常行chevron撤去、下中央Add task pill、Closed/Archived/チップ高さの既知nitsを含む | 軽量レーン | 出典: 2026-07-07親レビューのDesign Lab比較ギャップ分析。吸収: 親レビュー2026-07-06のdot整列 |
| 2 | task-44予定: Lucideアイコン統一 | 本番アイコンセットを `lucide_icons_flutter` へ統一し、Material Iconsとの混在を解消する。tooltip/semanticsは維持する | 軽量レーン | 出典: 2026-07-06人間裁定のLucide統一。task-43から分離 |
