...
背景: 2026-07-08にプロダクトオーナーがPhase 2の自律実装を承認した。本計画はその実装範囲と完了条件を定義する。初版は同期系に絞っていたが、同日に `docs/01_企画書.md` §8と整合するようカレンダー表示、タイマー/Pomodoro、テンプレート・繰り返しタスクを追加した。2026-07-17現在、P2-M1〜M4・M6・M7は完了した。P2-M5は削除同期、macOS / iOS Simulator確認、Android Rust FFI / Flutter release APK build、Android Keystore実装まで完了し、Android接続実機でのKeystore / Device Key rotation / DB reopen / 同期確認が残る。P2-M8は未着手である。一般リリースは別work itemの課金基盤を完成させるまで延期する。
...
- `docs/01_企画書.md` §8のうち、同期、アカウント、カレンダー、Pomodoro、Android Flutter release APK build、Android Keystoreは実装済みである。P2-M5のAndroid接続実機検証、P2-M8テンプレート / 繰り返し、課金release gateが残る。
