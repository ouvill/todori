...
- 進行中: なし。
- 最新の完了: **task-90 offline list作成 + key-bundle upload queue** — account-bound offline createでList DEK、local cache、list/state/outbox、bundle queueをatomicに確定し、preflight後・initial backfill/entity push前にimmutable bundleをuploadする。server成功後local ACK失敗からのsame-ciphertext retryと別client復号を含め、独立再検証でP1 / P2 / P3なしを確認済み。
