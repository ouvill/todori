...
- **Today/home**: 上部バー（メニュー/ソート） → Today見出し + 日付サブタイトル → リスト名pill → 単一のTasksパネル（見出し + pending pill + タスク行群。pending表示はここ1箇所のみ） → 控えめなClosed見出し → 画面下中央のpill型Add task FAB。通常タスク行の右端chevronは禁止し、行タップで詳細へ遷移できることはsemanticsで明示する。Todayスマートビューでは手動並び替えを行わない。
