...
- productionからDesign Lab / visual QA mockへのimportは現状存在しないが、`app/tool/check_client_boundaries.sh`にはこの依存方向を検出する明示的な静的検査がまだない。task-107の統合gateで追加する。
- Onboardingの3つの説明iconにMaterial Iconsが残る。Lucide統一時に同等glyphへ置き換え、同一画面内でicon familyを混在させない。
- Focus inverseのdestructive/error色`#F3A398`がwidget内に直書きされている。`AppFocusColors`へ意味tokenとして昇格してから再利用する。
