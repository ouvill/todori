...
- **次のUI較正タスク起票済み**: task-25で、AI生成画像・画像モックをピクセル完全基準にせず、既存実画面の密度、長いタイトル、i18n、Dynamic Type、狭い画面、タップ領域、tooltip/semanticsを較正する実装タスクを追加した。Undo・条件ソートUIへ進む前のUI実装判断タスクとして扱う。
- **テスト数**: Rust 70 / Flutter 27（いずれもtask-24完了報告と親検証時点の値。着手前に最新の完了報告で更新すること）。
...
| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | [task-25 design calibration UI pass](./task-25-design-calibration-ui-pass.md) | AI生成画像・画像モックへのピクセル追従ではなく、既存実画面の密度、長いタイトル、i18n、Dynamic Type、狭い画面、タップ領域、tooltip/semanticsを小さく較正する | M3 UI較正 | task-20〜24、`docs/design/visual-direction.md`、ユーザー懸念「visual direction完全追従では実アプリUIとして合成が取れない」由来。Undo・条件ソートUIより前に実施 |
| 2 | Undo | 削除/完了/編集のUndo。履歴データ構造、操作単位、復元時の競合方針を定めて実装する | M3-05残 | `docs/07_Phase1計画書.md` M3-05、task-24完了報告の分離事項 |
| 3 | 条件ソートUI | 手動順と締切/優先度/作成順ソートの切替UI。切替状態と設定保存方針も含めて整理する | M3-05残 | `docs/07_Phase1計画書.md` M3-05、task-24完了報告の分離事項 |
