...
- [x] pull failureが`MissingDek` / `NoMatchingDek` / `AuthenticationFailed` / 既知version corruption / unsupported envelope / unsupported protocol / invalid known-protocol responseへ型分類され、silent skipまたは汎用`Deferred`だけでcursorを進める経路がない。
- [x] protocol / envelope capability preflightがoutbox読取・pushより先に実行され、unsupported versionでは`UpgradeRequired`が観測でき、push / payload pull / outbox ACK / cursor更新が0件である。page内の未知envelopeもpage全rollbackと後続停止になる。
- [x] missing keyはSQLite write transactionを保持せずproduction key-bundle refreshを1回だけ行って同じpageを再試行する。refresh成功後に取得できたrecordは適用され、refresh失敗時はcursor不変、なお不足するrecordはmissing-key quarantineとなる。
- [x] missing-key / auth failure / corrupt plaintextのquarantine rowが受信encrypted blobまたはtagged tombstone、seq、record ID、collection、revision / semantic metadata、型付きreason、attempt / timestampを保持し、plaintext・鍵・tokenを含まない。同一record / seqの再処理は重複rowを作らない。
- [x] page内のapply / delete、remote HLC、record state、merge repush outbox、quarantine upsert / resolve、cursorが単一`BEGIN IMMEDIATE`でcommitされる。各write地点へfailureを注入するとdomain・state・outbox・quarantine・cursorがすべてpage開始前へrollbackする。
- [x] quarantineは後続key refreshまたは新しい有効recordで再適用でき、成功したdomain/state/repushとquarantine解決・削除がatomicである。再適用失敗はdataを失わず、unsupported versionはcursorを動かさずupgrade-requiredへ遷移する。
- [x] push conflict / superseded currentが処理不能な場合、stale local outbox headをACKせず、処理可能な場合だけcurrent適用またはreplacement head生成と同一transactionで進む。task-86 CASとtask-87 typed field mergeの既存release gateが継続成功する。
- [x] production/common adapterと実SQLite storeを通るrelease gateで、少なくとも2-clientまたは実HTTP/Postgres scenarioにより、missing DEK refresh成功、missing DEK継続quarantine + cursor前進、corrupt quarantine + cursor前進、unknown version停止、crash rollback、quarantine再適用を観測できる。fixtureがstore内部へ直接完了状態を注入しない。
- [x] key refresh / HTTPがlocal write transaction内で呼ばれないことをinstrumented testで確認し、同一pageの複数missing recordでもnetwork refreshが1回である。quarantine済み個別recordはpreflight成功後の無関係なlocal pushを永久停止させない。
- [x] schema migration、技術仕様、README共通品質ゲート、Docker/Postgresを含むworkspace test、必要なFlutter / FRB gate、`git diff --check`が成功し、独立verifierがP1 / P2なしの根拠を返す。
...
- 品質ゲート: `cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、Docker/Postgres込み`cargo test --workspace`（server sync 8件、client 22件、sync 50件、storage 68件成功/1件ignored、bridge 2件を含む）、bridge release build、`flutter analyze`、`flutter test`（124 passed / visual QA harness 1 skipped）、hardcoded-string check、`git diff --check`が成功した。
- Commits: `313b451`、`1d39772`、`67a2d1a`、`9395a9a`、`5afeb5e`、`cca808c`（本完了同期commitは後続）。
- 独立検証: 初回verifierはcandidate DEKとplacement binding、quarantine replay starvation、latest-head fence、dead summary fieldのP1 2件 / P2 2件を報告した。fixer commit `cca808c`後、同じverifierがコード、回帰test、task-86 / task-87 release gate、public/private境界を再確認し、P1 / P2 / P3なしで合格した。上記全品質ゲートもverifier自身が再実行した。
- Fixer追補: 独立検証の具体的指摘4件に限定して修正した。taskの復号candidateを`(list ID, DEK)`へ変更し、認証済みtyped placementとの一致を必須化したため、AからBへの移動はB鍵 + placement Bで適用でき、B鍵 + placement Cは既知taskの`AuthenticationFailed`または未知taskの`NoMatchingDek`として拒否される。quarantine replayは`missing_dek` / `no_matching_dek`だけを`(seq, record_id)` keyset paginationで100件ずつ全走査し、known corruptionを毎run再試行しない。100件のcorruptionより後ろにある101件目のmissing rowが1回のrefreshで再適用され、先頭100件のattempt countが増えないproduction adapter / real HTTP / Postgres testを追加した。quarantine putはcollectionをstate / outbox / quarantine間で不変とし、older seq無視、same seq / revisionは暗号state不変 + attempt加算、same seq / different revision拒否、newer seqだけreplaceへ固定した。値を返せないerror pathに存在したcore `SyncRunSummary.upgrade_required`は削除し、公開bridge statusの`upgrade_required`はdurable run errorを保持する`last_error`から観測する既存経路へ一本化したためFRB公開型変更とcodegenはない。修正後に`cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、Docker/Postgres込み`cargo test --workspace`（server sync 8件、client 22件、sync 50件、storage 68件成功/1件ignored、bridge 2件を含む）、bridge release build、`flutter analyze`、`flutter test`（124 passed / visual QA harness 1 skipped）、hardcoded-string check、`git diff --check`が成功した。fixerは合否判定とtask / `STATUS.md`完了化を行っていない。
