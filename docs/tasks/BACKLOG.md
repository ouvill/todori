...
- **2026-07-07ドッグフーディング第2回実施。** フィードバック11件をtask-44/45（作成）、task-46/47（予定）へタスク化。task-46/47以外の残件は後続バックログ補充で再確認する。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-44: チェックボックストグル一貫性 | 一覧・ネスト行・詳細画面サブタスク・アーカイブ済みリスト内のチェックボックスを常時トグル化し、Undoスナックバーを4秒自動消滅へ揃える | M3 polish follow-up | 出典: ドッグフーディング2026-07-07#2 |
| 2 | task-45: 階層ガイドと詳細画面ツリー調整 | サブタスク階層ガイドのL字/T字描画、詳細画面Subtasksの子孫ツリー表示、タイトル/ノート編集開始時のレイアウトがたつき解消を行う | M3 polish follow-up | 出典: ドッグフーディング2026-07-07#2 |
| 3 | task-46予定: Inbox自動プロビジョニング+既定リスト永続識別 | 初回/欠損時のInbox自動作成、既定リストの永続識別、v3マイグレーションを含めて設計・実装する | M4系 | 出典: ドッグフーディング2026-07-07#2 |
| 4 | task-47予定: Todayスマートリスト化+リスト一覧Todayリンク | Todayを全リスト横断のスマートリストへ移行し、リスト一覧にTodayリンクを追加する。セマンティクスは親提案を人間確認中: 全リスト横断で期日今日+期日超過、期日なしは対象外、TodayでのAdd taskはInboxに今日期日で作成 | M4系 | 出典: ドッグフーディング2026-07-07#2 |
| 5 | task-48予定: Lucideアイコン統一 | 本番アイコンセットを `lucide_icons_flutter` へ統一し、Material Iconsとの混在を解消する。tooltip/semanticsは維持する | 軽量レーン | 出典: 2026-07-06人間裁定のLucide統一。task-43から分離 |
...
| 9 | 設定値の永続化機構とF-01 UIモード設定の保存口 | 設定値を永続化する仕組みを用意し、F-01のUIモード選択の保存口を実装する | M4 | 出典: Phase1計画書§1（F-01「設定値の保存口のみ用意」）。通知設定・UIモードの前提。2026-07-06親棚卸しで確認 |
| 10 | アクセシビリティ検証パス | Dynamic Type / スクリーンリーダーラベル / コントラストの確認項目を通す | M4-03 | |
| 11 | 性能検証 | 1万件データで起動2秒以内・主要操作60fps・オフライン動作を計測し、結果を記録する | M4-04 / F-50〜F-52 | |
| 12 | 日付・時刻表記のロケール準拠リファクタ | 固定パターン `DateFormat('EEE, MMM d')` 等をskeleton API（`yMMMEd`等）へ置換し、ホストのロケール設定に従う | M4系 | 出典: 2026-07-06人間指示（`docs/design/ui-spec.md`参照）。ja表示「月, 7月 6」の不自然さもこれで解消 |
| 13 | オンボーディング/初回起動体験 | 範囲設計のplannerタスクから開始する。DK復旧不可の注意表示（計画書§5リスク表）を含む。マスコットの利用はvisual-direction.mdの方針に従う | M4系 | 出典: 2026-07-06人間裁定（要人間判断→確定） |
| 14 | iOSリリースビルド/署名/ストア提出準備 | macOS環境でReleaseビルドが成功し、ストア提出前のコンプライアンス確認項目を整理する | M5-01 | |
| 15 | Phase 1リリース前にthemeModeをライト固定 | ダークモード正式対応まではアプリの`themeMode`をlight固定にする。dark系トークン・コードは残置し、priority dot固定hexのコントラスト検証等の磨き込みはダークモード対応再開時（裁定により直近スコープ外）に行う | M5系 | 出典: 2026-07-06人間裁定 |
| 16 | macOS dogfoodingビルド配布 | macOS desktopで主要操作が通り、既知差分をリリースノートに記録する | M5-02 | |
| 17 | クラッシュレポート方針の確定 | F-53オプトイン文言・PII除去対象・実送信するかの判断を記録する | M5-03 | |
| 18 | SQLCipherクロスビルドのiOS/Android CI検証 | iOS/AndroidのSQLCipherビルド差分をCIで継続検証する | Phase1計画書§6 | |
