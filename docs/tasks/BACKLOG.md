...
- **2026-07-07ドッグフーディング実施。** フィードバック5件をtask-40/41（作成）とtask-42/43（後続）へタスク化。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-40: タスク一覧の再オープンとClosedサブタスク同伴 | Closed（done/wont_do）行の先頭コントロールタップでtodoへ再オープンし、閉じたサブタスクはClosedセクションへ分離せず親の下へ残す | 軽量レーン | 出典: ドッグフーディング2026-07-07 |
| 2 | task-41: リスト一覧のナビゲーション単純化 | リスト一覧行から「…」メニューとchevronを外し、リスト操作を開いたリスト画面右上overflowへ移設する | 軽量レーン | 出典: ドッグフーディング2026-07-07 |
| 3 | 詳細画面インライン編集（task-42予定） | タスク詳細の右上編集ボタン経由の編集を見直し、詳細画面上でより直接編集できる導線を設計・実装する | 軽量レーン | 出典: ドッグフーディング2026-07-07 |
| 4 | Design Lab準拠のビジュアル統一と遷移動線（task-43予定） | 本番画面をDesign Lab方向へ寄せ、画面遷移動線を整理する。既存のdot整列修正とLucide統一もこの作業へ吸収する | 軽量レーン | 出典: ドッグフーディング2026-07-07。吸収: 親レビュー2026-07-06のdot整列、2026-07-06人間裁定のLucide統一 |
