...
> ステータス: 完了（既存FTS5を没入型Searchへ接続し、独立検証合格）
> 作業日: 2026-07-13
...
- [x] BridgeServiceが既存FRB `searchTasks(query)`を薄く公開する。
- [x] 空または空白queryはbridgeを呼ばずidleになる。
- [x] 非空queryはdebounce中からloadingとして観測できる。
- [x] title / noteの各query termをprefix ANDとして検索する。
- [x] todo / in_progress / done / wont_doをすべて返す。
- [x] active listとarchived listのtaskを返し、削除済みtaskだけを除外する。
- [x] 結果へlist名とarchive状態のcontextを付与する。
- [x] 古いrequestの成功 / errorが新しいquery、または空queryの状態を上書きしない。
- [x] immersive Search routeが空query、loading、結果、0件、errorをsingle-canvasで表示する。
- [x] 結果からTask detailへ遷移し、戻るとqueryと結果を維持する。
- [x] keyboard操作、clear、back、tooltip / semantics、48px級tap targetを満たす。
- [x] 320px、390x844、日本語、text scale 2.0でoverflowや操作不能がない。
- [x] FTS schema / query、sync protocol、DB migration、新規依存を変更していない。
- [x] focused provider / widget test、Visual QA、共通品質ゲートが成功する。
- [x] 独立検証で検索範囲とstale-result protectionが合格する。
...
- 作業日: 2026-07-13
- bridge / provider: `BridgeService.searchTasks`を既存FRB呼び出しへの薄い1行の委譲として追加した。状態は`Idle` / `Loading(query)` / `Data(query, items)` / `Error(query, error, stackTrace)`で、debounceは250ms。空queryではbridgeを呼ばず、timer cancellationとgeneration / dispose guardで古い成功・失敗を破棄する。active / archived双方のlist名とarchive状態を結果へ合成する。
- 検索契約: storageのFTS schema、`build_fts_prefix_query`、`bm25`順を変更していない。title / noteの各termをprefix ANDで検索し、todo / in_progress / done / wont_doとarchive済みlistを含め、削除済みtaskだけを除外する。
- UI: Shell外のimmersive `/search` routeへ空query、loading、結果、0件、error / retryをsingle-canvasで実装した。Search専用detail childでqueryと結果を保持したまま往復できる。Home / Lists / list tasks / Youの48px search actionを見出しと同じ行に配置した。結果行はtitle、note、list / archive、status、priority dotを表示し、統合semanticsでもpriorityを読み上げる。
- 証拠: provider / Search widget 12件、全Flutter test 160件成功（Visual QA harness 1件は通常実行で意図的skip）。`app/build/visual_qa`でVisual QA 74 caseが成功し80 PNGを生成した。Searchのempty / loading / results / zero / error / archived+closed / 320px / text scale 2.0 / 日本語 / RTLを目視し、overflow・Shell navigation混入・操作不能がないことを確認した。
- 品質ゲート: `cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、Docker統合testを含む`cargo test --workspace`、Rust bridge release build、`flutter analyze`、`flutter test --concurrency=1`、hardcoded strings、client boundaries、`git diff --check`が成功した。最終priority semantics修正後もfocused 12件、analyze、境界checkを再実行した。
- Commits: `b04a2ea`、`16a4b36`、`29f6de3`。
- 未解決: なし。Calendar、Timer / Focusは後続taskで実装する。

### 独立検証

- 判定: 合格（HEAD `29f6de3`）。
- 根拠: 初回検証で結果行のpriority読み上げ欠落を不合格とし、修正と回帰test追加後に再検証した。storage / client / FRBが不変でbridgeが薄いこと、250ms debounce、空query no-call、stale-result guard、全status / archived list、削除除外、Search / detail往復を照合した。Cargo fmt / workspace clippy、storage FTS 5件、Search / provider 12件、Flutter analyze、hardcoded / boundary scripts、`git diff --check`を再実行して成功し、Search 10状態を含む80 PNGを確認した。
