...
> ステータス: 完了（wall-clock engineとproduction Focus experienceを独立検証済み）
> 作業日: 2026-07-13
...
- [x] 別IDのactive sessionを開始しても既存activeを置換せずtyped conflictになる。同IDの正当なpause / resume / add time更新だけを許可する。
- [x] Pomodoro既定25 / 5 / 15分、4 workごとのlong breakとlocal typed settingsが動作する。
- [x] Stopwatchがstart / pause / resume / finishでき、pause時間を実績へ含めない。
- [x] runningはwall-clock、pausedはaccumulatedだけから復元し、background / restart後も7日上限内で一致する。
- [x] target超過をresume時にsettleしてもendedAtはtarget到達instantになり、active durationと整合する。
- [x] completedまたは明示保存したinterrupted workだけを同期し、breakは同期しない。
- [x] Focus開始・pause・resume・通常finishでtask statusが変わらない。
- [x] Focus中task completeはsession保存成功後にだけ`done`となり、失敗時はstatus不変。UndoでTimerは再開しない。
- [x] dedicated Focus routeがsetup / running / paused / add time / finish / discard / restore / errorを表現する。
- [x] running / pausedのみdark inverse、setupと通常画面はwarm single-canvasを維持する。
- [x] open task trailing swipeがFocusを表示し、DueはTask detail propertyから編集できる。
- [x] Task detailでestimated / total actualが表示され、実績追加・sync・task削除後にinvalidateされる。
- [x] 通知permission拒否でもTimerは動作し、通知はbest-effort、kill中DB確定なし、resume時settleとなる。
- [x] semantics、44px以上のtap target、text scale 2.0、320px狭幅、日本語、RTL、Reduce Motionを満たす。
- [x] Rust testsでconditional active、state transition、restore、target instant、atomic finish、status不変を固定する。
- [x] Flutter testsでengine lifecycle、settings、notification、Focus全状態、swipe、detail集計、task complete / Undo、cache invalidationを固定する。
- [x] Visual QAで390×844、320px、720 / 1024px、setup / running / paused / finish / error、background復帰を確認する。
...
- 作業日: 2026-07-13
- 結果: conditional active lifecycle、25 / 5 / 15分・4 work周期のPomodoro、pauseを除外するStopwatch、wall-clock復元、7日境界、best-effort通知、restart-safeなwork completion / break handoff journalを実装した。FocusはShell外routeとなり、setupはwarm、running / pausedはdark inverse、open taskのtrailing swipeから開始できる。
- 順序保証: 通常のTimer操作はtask statusを変更しない。Focus内task完了はnon-zero work実績の原子保存成功後だけ`done`にし、Undoはtaskだけを戻してTimerを再開しない。
- UI: Focus setup / running / paused / add time / finish / break / discard / conflict / restore / error、Task detailのestimated / actual、Due property、deep-link時の安全なexitを接続した。
- 証拠: Rust workspace 290件成功、既定ignored 2件。Flutter 203件成功、Visual harnessの通常gate 1件は意図的skip。Focus visual 9状態は明示実行で9 / 9成功し、390×844、320px日本語text scale 2.0、720 / 1024px、finished、restored、errorを目視した。
- Commit: `4edf050`, `dbffcb4`, `6b1ab52`, `e4853ff`, `d8b6cf4`, `0506a18`, `0bef2d7`, `47fa846`, `2dd2afe`
- 未解決: exact alarm、background worker、Live Activityはscope外。次taskで`ui-spec.md`の実装済み状態との整合と全体統合仕上げを行う。

### 独立検証

- 判定: 合格（P0〜P2 findingなし）
- 根拠: 実装不参加の検証者がHEAD `2dd2afe`でCargo fmt / clippy / workspace、Docker server統合、release bridge、Flutter analyze / full test、hardcoded strings、client boundaries、diffを再実行した。Timer durationの完全一致、status直交、restart journal、finish-before-done、Undo非再開、Design Lab境界を監査し、Focus visual 9 PNGを目視した。
- 検証者: `/root/task104_ui_audit/duration_fix_verify`（read-only）
