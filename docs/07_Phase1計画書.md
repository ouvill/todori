...
| task-02 SQLCipher PoC | M1の前提。`core/storage` の暗号化DB、FTS5、repository実装に直結する | host / iOS Simulator core test、iOS実機target link、Android Rust FFI / arm64 release APK buildまで成功。非docs変更ごとのiOS no-codesign release buildとAndroid release APK buildをCIへ導入し、Pixel 7a / Android 16接続実機でDevice Key rotationとSQLCipher DB reopenも確認済み。iOS実機とAndroid同期の継続検証が残る |
| task-03 flutter_rust_bridge PoC | M2の前提。Flutter UIからRust coreを呼ぶ全機能の基礎 | `flutter_rust_bridge 2.12.0`のproduction API、macOS実行、iOS Simulator build / install / launch、Android Flutter release APK buildまで接続済み。Pixel 7a / Android 16のprofile E2EでRust bridge経由のDevice Key rotationを2回確認し、iOS実機とAndroid同期の検証が残る |
...
| M1-02 | SQLCipherスキーマをPhase 1対象テーブルへ拡張する | task-02 | `cargo test -p taskveil-storage` で暗号化DB、誤鍵失敗、FTS5、repository roundtripが通ること |
...
| リスク | 影響 | 対策または判断 |
|---|---|---|
| SQLCipherのiOS/Androidビルド差分 | target更新でリリース不能になり得る | iOS no-codesign release appとAndroid arm64 release APKを非docs変更ごとにCIでbuildし、両platformのRust bridge成果物も検証する |
| flutter_rust_bridgeのプラットフォーム組み込み | UIからcoreを呼べない | cargokit経由のmacOS実行、iOS Simulator build / install / launch、production UI起動まで確認済み。残るrelease確認はiOS実機とAndroid Flutter実機である。命名制約: crate名/pod名/FRB stemは `taskveil_app_bridge` で一致させる |
...
- SQLCipherとFRBのhost / iOS / Android基礎検証は成立し、iOS no-codesign release build CI、Android release APK CI、Android Keystore実装まで完了した。Pixel 7a / Android 16接続実機ではKeystore key non-exportability、Device Key rotation、DB reopenも確認済みである。残る継続検証はiOS実機とAndroid実機同期であり、一般リリースはさらに課金release gateへ依存する。
