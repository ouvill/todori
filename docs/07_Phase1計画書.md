...
| task-02 SQLCipher PoC | M1の前提。`core/storage` の暗号化DB、FTS5、repository実装に直結する | host / iOS Simulator core test、iOS実機target link、Android Rust FFI / arm64 release APK buildまで成功。Android release APK CIを導入済みで、iOS cross-build CIとiOS / Android実機継続検証が残る |
| task-03 flutter_rust_bridge PoC | M2の前提。Flutter UIからRust coreを呼ぶ全機能の基礎 | `flutter_rust_bridge 2.12.0`のproduction API、macOS実行、iOS Simulator build / install / launch、Android Flutter release APK buildまで接続済み。iOS / Android実機検証が残る |
...
| リスク | 影響 | 対策または判断 |
|---|---|---|
| SQLCipherのiOS/Androidビルド差分 | target更新でリリース不能になり得る | iOS Simulatorのcore test、iOS実機target link、Android Rust FFI / arm64 release APK buildは成功済み。Android release APK CIを導入済みで、iOS cross-build CIは別work itemで残る |
...
- SQLCipherとFRBのhost / iOS / Android基礎検証は成立し、Android release APK CIとAndroid Keystore実装まで完了した。残る継続検証はiOS cross-build CI、iOS / Android実機、Android接続実機でのKeystore / DB reopenであり、一般リリースはさらに課金release gateへ依存する。
