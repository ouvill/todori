...
> ステータス: 完了（single-canvas方向をproductionへ昇格）
> 作業日: 2026-07-11、再開・完了: 2026-07-13
...
- [x] `home_tasks.png` でHomeがInter主体のwarm single-canvasとして表示され、serif hero、白いpanel、通常task cardがない。
- [x] Homeの通常タスクが1件ずつ独立した大カードに見えず、セクション内で連続したリズムを持つ。
- [x] Quick AddとNavigationBarが別々の大きな帯として画面下を占有しない。
- [x] Homeの4期日セクション、各タスク最大1回表示、サブタスクツリー、完了Undoと完了モーションが維持される。
- [x] `lists.png` が短いindex mark、文字階層、hairlineで整理され、大きな空白と単一cardだけの画面ではない。
- [x] `task_detail.png` で大きな外周カードとpill群が主役にならず、タイトル・note・属性・subtasksがdocumentとして読める。
- [x] `account_signed_out.png` でServer URLが最初の巨大フォームに見えず、接続設定とログイン操作の優先順位が明確である。
- [x] ワイドHome / Listsで本文が横幅いっぱいに伸びず、読みやすい最大幅または複数pane構造になる。
- [x] 通常画面の見出し、本文、タスク、操作はInterに統一され、Newsreader / Source Serif 4 / システム明朝をproductionの情報階層に使わない。
- [x] 通常画面でcard / pillを階層表現として常用せず、pillは選択状態や短い一時的操作など形状に意味がある場合だけに限定する。
- [x] Focus以外のproduction画面にdark inverse surfaceがなく、通常headerへマスコットを常設しない。
- [x] Design Labがfake data専用のままで、production codeからDesign Lab / visual QA mockへのimportがない。
- [x] 完了motionがpress 90ms級 → fill約200ms → 130ms後からcheck path約330ms → 単一halo約520ms → strikethrough → 500ms保持 → 420ms collapseの順で動き、Reduce Motionでは装飾遅延なしに確定する。
- [x] 390x844、日本語、text scale 2.0でoverflowや操作不能がない。
- [x] tooltip、semantics、48px級タップ領域、Reduce Motionを維持する。
- [x] Flutter品質ゲート、境界check、`git diff --check` が成功する。
- [x] before / afterの全visual QA PNGを目視し、画面単位の所見を完了報告へ記録する。
- [x] Search / Calendar / Focusの本番機能やデータAPIを本タスクの外観変更に混入させていない。
...
- Focus setupは円形dialを廃止し、typographic duration controlへ変更した。Focus中はdark forestの専用画面へ遷移し、円形progressを使わず、マスコットbirdが水平な時間軸上を進むTaskveil固有のprogress表現へ変更した。
...
- 既存のマスコットkit `assets/brand/generated/taskveil-mascot-kit-refined-no-border.png`をcharacter identityの正本として、Focus飛行・完了・休息の3ポーズを同じアイリング、喉色、羽色、手描き質感で再構成し、chroma key除去済みsprite sheet `taskveil-mascot-ui-sprites-v1.png`を作成した。通常headerのLucide bird + `TASKVEIL`は撤去し、default Design LabではFocusの時間軸だけに実物のツグミドリ飛行poseを表示する。
...
- bottom navigationは文字だけの実験案から、控えめなLucide icon + 小さなlabel + active underlineへ変更した。中央captureだけを緑の円形主操作とし、判別性とTaskveilの静かな強度を両立する。
...
### Production昇格（2026-07-13）

- 結果: Design Labで裁定したInter主体、`#F8F5EC`のwarm single-canvas、必要最小限のhairline、低角丸、Focusのみdark inverseという5軸をproduction themeと主要画面へ昇格した。Home / Lists / Task detail / Account / Onboarding / system states / sheetから通常行card、情報pill、serif階層を除き、モバイルはHome / 中央Capture / Lists / You、ワイドはcompact railと既存最大幅へ統一した。
- Home / motion: Calendar完成前のOverdue / Today / Tomorrow / Upcoming、各task最大1回、3階層subtask tree、Undoを維持した。完了はpress 90ms、fill 200ms、130ms後からcheck 330ms、halo 520ms、strikethrough、500ms保持、420ms collapseで進み、Reduce Motionは即時確定する。
- Capture: 初回独立検証でList / Dueのoutlined pillとtext scale 2.0時の欠けを指摘された。縦積みの52px以上のhairline property rowへ修正し、390x844、text scale 2.0、320px狭幅で値・chevron・操作が収まることを再撮影した。Plan / Priorityは後続taskへ残し、本taskには混入させていない。
- 依存境界: `app/lib`から`design_lab` / `visual_qa`へのimportがないことを検索で確認した。Design Labはfake data専用の独立環境として維持し、Search / Calendar / Focusのproduction機能やデータAPIは追加していない。
- Visual QA: before=`app/build/visual_qa_before_task100_promotion/`、after=`app/build/visual_qa/`。58 test成功、64 PNGを目視した。Home英日・空・text scale 2.0・wide、Lists通常・archived・wide、Task detail通常・editing・text scale 2.0、Account、Onboarding、sheet / dialog / swipe / completion midframeに描画異常なし。Task detailはcanvasを上下端まで明示的に塗り、Captureは通常幅・text scale 2.0・320pxで欠けなし。
- 品質ゲート: `cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、Docker統合testを含む`cargo test --workspace`、Rust bridge release build、`flutter analyze`、`flutter test --concurrency=1`（143成功、Visual QA harness 1件は通常実行で意図的skip）、hardcoded strings、client boundaries、`git diff --check`が成功した。
- Commits: `2f08bdf`、`85e2559`、`e3a5b0e`、`a87a158`、`f6262fd`、`83d8ae3`、`6ba3021`、`a0bedc3`、`6121775`。
- 未解決: なし。Plan / Priorityを含むCapture属性、Search、Calendar、Timer / Focusは承認済み後続工程として別taskで実装する。

### 最終独立検証

- 判定: 合格。
- 初回指摘: Capture sheetのList / Dueが大きなoutlined pillで、text scale 2.0時に左右の操作が欠ける2点を不合格とした。
- 再検証: HEAD `6121775`で縦積みhairline property row、52px以上のtap領域、通常幅・text scale 2.0・320px狭幅の欠けなしを確認した。`flutter analyze`、Capture関連widget test 7件、`git diff --check`が成功し、worktreeがcleanであることを確認した。その他の主要画面、wide navigation、Design Lab境界、完了motion / Reduce Motionは初回検証から合格。
- 検証者: 実装非担当サブエージェント `/root/task100_verifier`。
