...
5. 旧パス `/Users/youhei/workspaces/taskveil/app/rust_builder/cargokit/build_tool` がどのファイルに残っているか検索する。
...
3. `rg -n "/Users/youhei/workspaces/taskveil|build_tool/build_tool.dart|build_tool_runner" app .github` で古いパスと生成物参照を調べる。
...
- [ ] 旧絶対パス `/Users/youhei/workspaces/taskveil/app/rust_builder/cargokit/build_tool` の発生源と現在の残存有無が記録されている。
...
- 生成された `build_tool/pubspec.yaml` の `build_tool` path dependency が、存在しない旧絶対パス `/Users/youhei/workspaces/taskveil/app/rust_builder/cargokit/build_tool` を参照している `path_does_not_exist`
...
`app/rust_builder/cargokit/run_build_tool.sh` は `BUILD_TOOL_PKG_DIR="$BASEDIR/build_tool"` を `pubspec.yaml` の path dependency として書き出す。今回残っていた `app/build/macos/.../build_tool/pubspec.yaml` は、リポジトリ移動前の `/Users/youhei/workspaces/taskveil/...` を埋め込んだ古い生成物だった。
...
- `app/pubspec.yaml`: `taskveil_app_bridge` は `path: rust_builder` のままで、旧絶対パスは含まれていないことを確認した。
...
旧絶対パス `/Users/youhei/workspaces/taskveil/app/rust_builder/cargokit/build_tool` は、macOS build artifact 内の以下の生成 `pubspec.yaml` に残っていた。

- `app/build/macos/Build/Intermediates.noindex/Pods.build/Debug/taskveil_app_bridge.build/build_tool/pubspec.yaml`
- `app/build/macos/Build/Intermediates.noindex/Pods.build/Debug/rust_lib_taskveil.build/build_tool/pubspec.yaml`
