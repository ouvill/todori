...
`core/`はworkspace内の配置ディレクトリでありcrate名ではない。実packageは`taskveil-client`等、Rust crate名は`taskveil_client`等で、Rust標準の`::core`との競合はない。
...
- SQLite sync adapterをFlutter bridgeから`taskveil-client`へ移し、Fuzzy-scanのSQLite実装先を共通clientへ固定する。
- server integration testからFlutter bridgeへの逆依存をなくす。
- CLI/MCPのTaskveil workspace内依存入口を`taskveil-client`へ統一する。
...
- server integration testを共通client型へ切り替え、`taskveil_app_bridge` dev-dependencyを除去する。
- app側の既存support参照は、実装を持たない一時compat aliasだけに縮小する。
- CLI/MCPのdomain/storage直接依存を`taskveil-client`へ置き換える。
- architecture boundary check、技術仕様、ADR-011、AGENTS、architecture文書を更新する。
- bare `core` package/lib/dependency aliasを禁止し、package `taskveil-<role>` / crate `taskveil_<role>`を規約化する。
...
- [x] server integration testの`taskveil_app_bridge` dev-dependencyがなくなり、共通clientの`SqliteSyncStore`を利用する。
- [x] CLI/MCPがdomain/storage/sync/cryptoへ直接依存せず、`taskveil-client`を通常依存に持つ。
...
- `[package] name = "core"`、`[lib] name = "core"`、dependency alias `core = {...}`、曖昧なumbrella `taskveil-core` crateを追加しない。
- `taskveil_app_bridge`はCargo package / lib target / FRB stem / pod名の固定契約として例外的にunderscore名を維持する。
...
- Server依存: `server/tests/sync_v2.rs`は`taskveil_client::SqliteSyncStore`を使用し、`server/Cargo.toml`と`Cargo.lock`から`taskveil_app_bridge` dev-dependencyを削除した。real Axum HTTP + Docker/Postgresを通るsync v2 9件は継続成功した。
- CLI / MCP: `taskveil-cli`と`taskveil-mcp-server`のdomain/storage直接依存を外し、通常依存を`taskveil-client`へ統一した。両binaryはまだ機能stubであり、実profile openは後続taskとした。
- 境界固定: `app/tool/check_client_boundaries.sh`をCIと共通品質ゲートへ追加した。CLI/MCPの`taskveil-client`以外のTaskveil crate依存、`api.rs` / `support.rs`以外のbridge sourceにおける下位import/SQLCipher/sync実装、legacy 2ファイルの下位参照数が現状94件から増える変更、sync store compat aliasの肥大化、bare `core` package/lib/dependency aliasを検出する。
- 命名: `core/`はディレクトリ、Cargo packageは`taskveil-<role>`、Rust crateは`taskveil_<role>`と文書化した。`cargo metadata`上もbare `core` targetは存在しない。`taskveil_app_bridge`はCargo / pod / FRB stem契約の例外として維持する。
...
- 判定: 合格（P1 / P2 / P3なし）
- 根拠: 旧bridge実装と新`SqliteSyncStore`を型名正規化してdiff 0、serverからapp bridge依存なし、CLI/MCPの下位crate直接依存なし、FRB/schema/wire/crypto/UI差分なしを確認した。boundary checkへ禁止source import、CLIの`taskveil-storage`依存、bare `core`名、sync store再肥大化を一時fixtureとして個別注入し、すべて期待どおり非0終了、復元後に成功した。`cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、Docker/Postgres込み`cargo test --workspace`、bridge release build、`flutter analyze`、`flutter test`（130 passed / visual QA harness 1 skipped）、hardcoded-string check、client boundary check、`git diff --check`を独立再実行して成功した。秘密情報露出とpublic/private境界違反はなかった。
