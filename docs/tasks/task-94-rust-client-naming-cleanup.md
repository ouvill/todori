...
> ステータス: 完了（TaskveilClient命名統一・旧aliasなし・独立検証合格）
> 作業日: 2026-07-10
...
同時に、`taskveil-client`内部にはtransactional mutation専用の低水準`Client`が残り、高水準入口を`TaskveilClient`へ改名した際に役割が衝突する。Taskveilは未リリースで内部互換を要求しないため、旧名aliasを残さず、実行主体とローカル保存境界を分離した最終命名へ直接置き換える。
...
- frontend共通の高水準runtime facadeを`TaskveilClient`と呼ぶ。
...
- Flutter、CLI、MCPが引き続き`taskveil-client`だけを入口とし、Fuzzy-scan実装前の境界を明瞭にする。

## 4. スコープ

### やること

- `ClientProfile` → `TaskveilClient`、`ProfileConfig` → `LocalProfileConfig`へbreaking renameする。
...
- [x] 通常public APIの唯一の高水準入口が`TaskveilClient`で、`LocalProfileConfig`を受け取る。
...
- [x] CLI/MCPが`TaskveilClient::open(LocalProfileConfig)`とasync APIをcompile時に固定する。
...
- [x] 現行architecture文書が`TaskveilClient`と`LocalProfile`の違いを明記する。
...
- crate名`taskveil-client` / `taskveil_client`とFRB固定名`taskveil_app_bridge`は変更しない。
...
- 作業日: 2026-07-10
- 結果: frontend共通runtime facadeを`ClientProfile`から`TaskveilClient`へ、起動設定を`ProfileConfig`から`LocalProfileConfig`へbreaking renameした。旧名aliasは追加していない。
...
- Public API: 通常root公開面は`TaskveilClient`、`LocalProfileConfig`、frontend-neutral model / command / domain型、`ClientError`に限定した。`SqliteMutationService`、`LocalMutationContext`、SQLite sync store、local crypto helperは引き続き`test-support`限定である。
- 契約: CLI/MCPは`TaskveilClient::open(LocalProfileConfig)`とasync `sync_now`をcompile時に参照する。bridgeの39公開関数signature testは成功し、FRB 2.12.0再生成後のDart差分は`ClientProfile`を`TaskveilClient`へ直すdoc comment 1行だけだった。
...
- Documentation: `LocalProfile`を端末内データ/security boundary、`TaskveilClient`をruntime facade、`LocalProfileConfig`を起動設定、`LocalProfileBinding`を永続account identityとして区別し、AGENTS、技術仕様、ADR-011、architecture文書へ同期した。
