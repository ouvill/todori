...
- Rust型:
  ...
  - `TaskUndoEntry`
- snapshot形式:
  - `todori_domain::Task` 全体を `serde_json` で `before_snapshot` に保存する。
  - 新規依存は追加せず、workspace既存の `serde_json` を `todori-storage` から参照した。
...
- `todori-private/` は読んでおらず、public repoにprivate詳細は転記していない。
