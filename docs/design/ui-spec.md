...
- **Home**: ルート画面はTodayではなくHomeである。ブランドsprout / ソート → 小さなInterの日付キッカー → 42px級セリフの `Home` 主見出し → Overdue / Today / Tomorrow / Upcoming のセクション群 → 画面下部常設クイック追加バー。日付を主見出しにせず、画面の場所をHomeとして明示する二段構成とする。旧ハンバーガーメニュー、画面下中央のpill型Add task FAB、入力ダイアログは廃止する。通常タスク行の右端chevronは禁止し、行タップで詳細へ遷移できることはsemanticsで明示する。Home横断ビューでは手動並び替えを行わない。
...
- **リスト一覧**: Listsはグローバルナビゲーションから直接開くトップレベル領域とする。旧戻る矢印、Home行、Account overflowを置かない。行は純粋なナビゲーション行とし、行内に操作メニューやchevronを置かない。リスト単位の操作（改名/アーカイブ/削除）は、そのリストを開いた画面の右上overflowメニューに置く。既定インボックスでは保護対象操作（削除/アーカイブ）をメニューに表示しない。この規則は2026-07-07ドッグフーディングおよび2026-07-11 IA再設計由来である。
...
- モバイルのNavigationBarは72px級、surface背景、primaryContainerの選択indicator、Lucide icon、短いラベルで構成する。NavigationRailは同じ3項目とsprout brand markを持ち、contentとの境界は1px `outlineVariant`とする。
