...
Phase 1の先行プラットフォームはiOSとする（`docs/01_企画書.md` §8、`docs/06_事業・法務方針.md` §6）。開発ホストはmacOSであるため、iOSビルドとSimulator/実機確認をローカルで実施できる。日常のdogfoodingと自動テスト用にはmacOSデスクトップビルドを維持する。

| 分類 | Phase 1に含める機能 | 判断 |
|---|---|---|
| UI | 初回はシンプルUI固定に近い導線で開始し、F-02を実装する。F-01のUIモード選択は設定値の保存口のみ用意し、高機能UI（F-03）への実画面遷移はPhase 3へ送る | MVPの迷いを減らすため |
| タスク管理 | タスクCRUD（F-05）、サブタスク、`done` / `wont_do` / 再オープン（F-06）、ゴミ箱・Undo（F-07）、並び替え（F-08） | 企画書§8の中核 |
| リスト | リストCRUDと既定インボックス（F-09） | ローカルTodoとして必須 |
| タグ | F-10はPhase 3へ送る | 企画書§8のMVP記述外で、検索/フィルタUIの複雑度が高い |
| 全文検索 | F-11は技術基盤のみM1で検証し、ユーザー向け検索UIはPhase 3へ送る | SQLCipher + FTS5の技術リスクは早期に潰すが、MVP操作面からは必須ではない |
...
Phase 1では、同期・アカウント機能（F-27〜F-31）、Organization共有（F-32〜F-35）、課金（`docs/04_課金設計書.md` 全体）、カレンダー表示（F-13）、Pomodoro/タイマー（F-16〜F-18）、テンプレート・繰り返し（F-19〜F-21）、ローカルAI（F-36〜F-39）、MCP/CLIの実機能（F-40, F-41）は実装しない。`cli/` と `mcp-server/` の雛形crateは存在するが、Phase 1ではローカルDB実操作の接続対象にしない。
...
| task-02 SQLCipher PoC | M1の前提。`core/storage` の暗号化DB、FTS5、repository実装に直結する | `rusqlite 0.40.1` + `bundled-sqlcipher-vendored-openssl` でホストビルドとテスト成功。Android/iOSは別途CI/実機検証が必要 |
| task-03 flutter_rust_bridge PoC | M2の前提。Flutter UIからRust coreを呼ぶ全機能の基礎 | `flutter_rust_bridge 2.12.0` で `greet` と `create_draft_task` の垂直貫通を作成済み。各プラットフォームのビルド組み込みは未完了 |

補足（2026-07-04 iOS実行検証）: task-01/02の成果物をiOS環境で直接検証した。`aarch64-apple-ios-sim` 向けクロスビルド（vendored OpenSSL/SQLCipher含む）が成功し、iOS Simulator上で `todori-crypto` 全17テスト（OPAQUE一気通貫・Argon2含む）と `todori-storage` 全10テスト（SQLCipher暗号化DBのファイルI/O・FTS5・Device Key導出鍵でのopen含む）が成功した。実機ターゲット `aarch64-apple-ios` も `IPHONEOS_DEPLOYMENT_TARGET=15.0` 指定（`.cargo/config.toml` に設定済み）でリンク成功。これによりiOS上の暗号・ストレージ実行リスクは解消し、残るiOS関連リスクはFlutterビルドパイプラインへのリンク組み込みのみとなった。
...
| ID | 内容 | 依存タスク | 完了条件 |
|---|---|---|---|
| M5-01 | iOSビルド/署名/ストア提出準備を整える | M4 | macOS環境でReleaseビルドが成功し、暗号輸出チェック項目が整理されていること |
...
| リスク | 影響 | 対策または判断 |
|---|---|---|
| SQLCipherのiOS/Androidビルド差分 | Phase 1 iOSリリース不能 | iOS: Simulator上で全コアテスト成功、実機ターゲットもリンク成功を確認済み（2026-07-04、§3補足参照）。残作業はFlutter/Xcodeへのリンク組み込み。Android: Rust libraryとしてのコンパイル成功まで確認済み（task-02） |
| flutter_rust_bridgeのプラットフォーム組み込み | UIからcoreを呼べない | cargokitを採用しmacOSで実機動作確認済み（2026-07-04）: `app/rust_builder`（FFIプラグイン）のpodspecがXcodeビルド中にcargoを実行しstaticlibをリンクする構成で、`flutter build macos` → アプリ起動 → サンドボックス内にSQLCipher暗号化DB生成まで確認。iOS用podspecも同梱済みで、残作業はiOS Simulator/実機での `flutter run` 確認。命名制約: crate名/pod名/FRB stemは `todori_app_bridge` で一致させる必要がある |
...
| タグ/全文検索/アプリロックの範囲膨張 | MVP完了が遅れる | タグと検索UIはPhase 3、アプリロックはPhase 1.5候補へ送る。FTS5技術基盤のみM1で保持する |
| パスワード紛失UX | Phase 1ローカル専用では対象外 | OPAQUE/Recovery KeyはPhase 2で扱う。Phase 1ではDK/OSキーチェーン復旧不可の注意表示を検討する |

## 6. 既存仕様書との差異・確認事項

- `docs/01_企画書.md` §8はPhase 1概要に全文検索・タグ・アプリロックを明記していない一方、`docs/02_機能仕様書.md` の対象F番号には含まれる。本計画ではMVPの中核度と実装リスクから、タグ・検索UI・アプリロックを次フェーズ側へ送る。
- `docs/03_技術仕様書.md` §5.1はSQLCipher採用を前提とするが、§12ではプラットフォーム別ビルド検証が未決事項である。ホストPoCは成功したため、残りはiOS/Android CI検証に絞る。
