...
- Design Labのtask完了を、90msのpress、約200msの先行fill、130ms後から330msかけて描くcheck path、520msの単一halo、105ms後から315ms進むstrikethroughへ引き上げた。完了taskは800msだけ通常位置に留まって操作の結果を見せた後、420msかけて行高を連続的に縮め、fadeしながら4px上へ抜けてCompletedへ移る。後続taskは同じheightFactorへ追従して滑らかに詰まる。Reduce Motionでは装飾motionを省略して即時確定する。
