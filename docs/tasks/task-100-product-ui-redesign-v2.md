...
- Design Labのtask完了を、productionと同じ時間設計の250ms check fill / path、500msの8方向particle、300msの左から右へ進むstrikethroughへ引き上げた。完了taskは800msだけ通常位置に留まって操作の結果を見せた後、Today streamから退いてCompletedへ移る。Reduce Motionでは装飾motionを省略して即時確定する。
- completion motion導入後に静止時のcheckが太く主張して見えたため、外周を1.0px、check pathを1.4pxへ抑え、fillを低彩度のgreenへ変更した。particleと時間設計は維持し、操作時の楽しさと通常一覧の静けさを分離した。
...
- 検証結果: Interactive Design Lab widget test 2件、Flutter全体133件が成功（visual QA harness 1件は設計どおりskip）。Design Lab 16画面・20 PNGを再生成し、icon + label navigation、CalendarのCompleted開閉、task capture sheetの4属性、completion midframe、Focus horizon、connectorとcheckboxの間の余白に加え、list task通常／menu press／timer reveal、task editing、account access、action / due sheet、system states、onboardingを目視確認した。専用entrypointのiOS Simulator debug build、`flutter analyze`、hardcoded strings check、client boundary check / test、`git diff --check`も成功した。
