...
ユーザーは、TaskveilのOSS公開運用として「public repoを主、private repoを内部メモ置き場」とする方針を選択した。つまり、コード・ビルドに必要な設定・公開可能な技術仕様はpublic `taskveil` を正本とし、課金・法務・未確定ロードマップ・公開前の荒い設計メモ・セキュリティ監査メモなどはprivate側で育て、publicへ出す場合は要約・抽象化して転記する運用である。
...
Taskveilを「public repoを主、private repoを内部メモ置き場」で運用するため、以下を明文化する。
...
| 5. プロダクト名 | public要約 + private詳細 | 名称Taskveil、由来、ブランド方向 | 候補比較、商標/ストア/ドメイン/SNS調査メモ |
