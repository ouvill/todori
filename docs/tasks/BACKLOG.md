...
- **2026-07-08ドッグフーディング第5回実施。** フィードバック3件（ツリー表示のチェック丸と縦線ずれ、チェック時の楽しさ、未チェック円線の太さ）はtask-56へタスク化済み。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-56: チェックボックスpolish | 階層ガイドを親チェックボックス中心基準へ揃え、未チェックリングを細くmutedにし、チェックON/OFFのマイクロモーションを調整する | 軽量レーン | 出典: ドッグフーディング2026-07-08#5 |
| 2 | 自然言語日付入力の解析 | クイック追加バー入力中の `tomorrow` / `next Friday` / `明日` 等を日付として解釈する | 将来枠 | 出典: 2026-07-07 Home裁定。task-52ではスコープ外 |
