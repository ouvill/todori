...
- **2026-07-07ドッグフーディング第2回実施。** フィードバック11件のうち、task-44/45は完了、task-46は指示書作成済み、task-47は予定としてタスク化。task-47以外の残件は後続バックログ補充で再確認する。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-46: Inbox自動プロビジョニング+既定リスト永続識別 | 初回/欠損時のInbox自動作成、既定リストの永続識別、v3マイグレーションを含めて設計・実装する | M4系 | 出典: ドッグフーディング2026-07-07#2 |
| 2 | task-47予定: Todayスマートリスト化+リスト一覧Todayリンク | Todayを全リスト横断のスマートリストへ移行し、リスト一覧にTodayリンクを追加する。セマンティクスは親提案を人間確認中: 全リスト横断で期日今日+期日超過、期日なしは対象外、TodayでのAdd taskはInboxに今日期日で作成 | M4系 | 出典: ドッグフーディング2026-07-07#2 |
