...
Todoriはpre-releaseであり、初回stable releaseまでは `main` branch のみをセキュリティ修正対象とする、と明記した。stable release後のバージョンサポート方針は、releaseが存在する段階で本文書に追記する扱いとした。
...
- `rg --files -g 'SECURITY.md' -g '.github/SECURITY.md' todori`: 変更前は該当なし、変更後は `todori/SECURITY.md` を確認。
- `rg -n "security|vulnerab|脆弱|SECURITY|issue|private vulnerability" todori/README.md todori/CONTRIBUTING.md todori/docs/tasks/README.md`: 変更前はtask一覧のみ、変更後はREADME/CONTRIBUTING導線を確認。
