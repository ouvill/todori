...
| task-23 | 未着手 | [task-23-trash-restore-ui.md](./task-23-trash-restore-ui.md) | ゴミ箱画面・復元UI。既存の `get_trashed_tasks` / `restore_task` / `trash_task` を使い、削除済みタスク一覧と復元導線を追加する | task-18〜22の成果物に依存。BACKLOG上はM3-04相当、計画書上はM3-02の削除/復元残りにも対応 |
