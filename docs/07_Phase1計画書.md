...
| ID | 内容 | 依存タスク | 完了条件 |
|---|---|---|---|
| M5-01 | iOSビルド/署名/ストア提出準備を整える | M4 | macOS環境でReleaseビルドが成功し、ストア提出前のコンプライアンス確認項目が整理されていること |
...
- 2026-07-07仕様改訂（ゴミ箱廃止・削除は恒久削除＋不可逆警告の追加確認・削除Undoなし・完了/編集Undoは維持・履歴保全経路としてリストのアーカイブをPhase 1で導入）により、`docs/02_機能仕様書.md` F-07 / F-09を改訂した（`docs/05_設計判断記録.md` ADR-009参照）。既存M3マイルストーン（M3-02等）の過去の完了記録・完了条件文言は書き換えず、新仕様への追従（ゴミ箱撤去・リストアーカイブ実装）は別途タスクとして`docs/tasks/BACKLOG.md`で管理する。
