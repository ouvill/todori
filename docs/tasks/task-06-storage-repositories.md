...
5. **統合テスト**（`core/storage` 内、`#[cfg(test)]` モジュールに追加。暗号化DBは `open_encrypted` を使用し、`tempfile::NamedTempFile` で分離すること。`tempfile` は既存の dev-dependency である）。少なくとも以下を含めること。
   ...
   - (2) `taskveil_domain::usecases` と接続した一気通貫テスト: `new_list` で作成したリストを `ListRepository::insert`、`new_task` で作成したタスクを `TaskRepository::insert` し、`update_title` / `transition_task`（`Done` への遷移）の結果を `TaskRepository::update` で永続化したのち、DBを再オープンしてもその変更が反映されていること。
...
- 新規依存クレートの追加。特に**ネットワークアクセスを要する新規クレートの追加は禁止**する。`rusqlite` / `hex` / `tempfile` / `taskveil-domain` は既存依存を再利用すること。本タスクの実行環境はネットワークアクセス不可であるため、crates.ioからの新規取得が発生する変更を行ってはならない。
...
7. `core/domain::usecases` を `dev-dependencies` からではなく通常の `taskveil-domain` 依存（既存）経由で使い、統合テストを書く。テストは `core/storage/src/lib.rs` の既存 `#[cfg(test)] mod tests` に追記する形でよい。
8. `cargo test -p taskveil-storage` を繰り返し実行しながら実装する。
...
- [ ] `cargo test -p taskveil-storage` で本タスクの新規テストがすべて実行され成功する
...
- `cargo test -p taskveil-storage` 成功（9 tests）。
