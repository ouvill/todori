...
一方、AI生成画像や画像モックのvisual directionに完全追従すると、実アプリUIとしての密度、長いタイトル、i18n、Dynamic Type、狭い画面、タップ領域、tooltip/semantics、既存操作との整合が崩れる可能性がある。画像はTaskveilらしさを共有する参照素材であり、実装正解やピクセル完全基準ではない。
...
- `taskveil-private/` 配下を読んだり変更したりしない。public repoにprivate側の課金、収益、法務、監査、公開前ロードマップ詳細を転記しない。
...
- `git -C taskveil status --short`: 変更ファイルのみ表示（無関係差分なし）。
...
- `cd taskveil && sh app/tool/check_hardcoded_strings.sh`: 成功。
...
- 2026-07-05に親Codexが変更ファイル、完了報告、禁止範囲、git状態を確認した。
- 別検証セッションで `dc2661c4e8c58501c6c30101b9c873ba63e4198a` を対象に受け入れ基準を再確認し、合格判定を得た。
- 独立検証では `git -C taskveil diff dc2661c^ dc2661c --check`、`cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、`cargo test --workspace`、`cd app && flutter analyze`、`cd app/rust && env CARGO_TARGET_DIR=target cargo build --release`、`cd app && flutter test`、`sh app/tool/check_hardcoded_strings.sh` が成功した。
