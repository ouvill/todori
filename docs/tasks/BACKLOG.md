...
| 3 | DBスキーママイグレーション機構の整備（task-36） | `core/storage` にスキーマバージョニングとマイグレーション実行機構を整備し、誤鍵/旧版DBのテストを追加する | M4前提（core/storage） | 出典: 2026-07-06人間裁定（要人間判断→確定）。設定値の永続化（#10）を含む以後のスキーマ変更をすべてマイグレーション経由にするための前提。task-37 リスト削除の前提（`lists.deleted_at` 追加が最初のマイグレーション題材） |
| 4 | リスト削除UI（論理削除+完了履歴保全+未完了ゴミ箱送り+復元時inbox remap）（task-37） | リストの論理削除（`lists.deleted_at`）を実装する。削除時、完了済みタスク（done/wont_do）は削除済みリストに紐付いたまま保全し、未完了タスクはゴミ箱へ送る。ゴミ箱からの復元時、元リストが削除済みなら既定インボックスへ`list_id`をremapする | M3-01 | 出典: M3-01 + 2026-07-07人間裁定（データ保持原則）。task-36（DBスキーママイグレーション機構）完了後に着手 |
