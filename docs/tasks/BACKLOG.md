...
| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-58: Home完了タスクの単独表示抑止 | 完了した期日あり子孫をOverdue/Today等へ単独表示せず、表示中祖先配下の同伴/完了ルートのClosed/祖先非表示サブタスクの非表示へ整理する | M3 polish follow-up | 出典: 2026-07-08ドッグフーディング（完了済み期日超過サブサブタスクのOverdue残留）。指示書作成済み |
