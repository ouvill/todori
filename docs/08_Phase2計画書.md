...
| 課金外部E2E未完 | 購入・server-side entitlement・失効認可のコードは実装済みだが、価格承認、provider設定、App Store商品設定、実機sandbox証跡が未完である | Billing foundation release gateを完了するまでstore提出、release tag、公開告知を行わない |
...
- 課金provider、product、trial / grace、restore方針は承認済みで、Billing foundationのコード実装と独立検証は完了した。価格承認、RevenueCat Test Store、App Store商品設定、Apple sandbox実機E2Eを完了する。
