...
| 層 | 所有するもの | 所有しないもの |
|---|---|---|
| `todori_app_bridge` | FRB公開関数、文字列/typed input変換、Dart向けDTO変換、process内profile handle | repository、SQLCipher open、鍵、account/sync state、同期順序 |
...
task-91時点の段階移行では、`app/rust/src/api.rs`と`support.rs`に既存の下位crate直接参照が残る。この2ファイルだけを期限付きlegacy exceptionとし、他のbridge sourceへ実装を広げること、およびlegacy参照数を現状から増やすことをCIで禁止する。次taskで両ファイルのapplication/profile責務を`ClientProfile`へ移し、exceptionとapp manifestの下位crate依存を同時に削除する。Fuzzy-scanはこの移設完了後に実装する。
