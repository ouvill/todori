...
- [x] full resyncにstable-key fuzzy scan、`base_seq`、delta catch-up、page transactionのhigh-water closure、mark-and-sweepが定義されている。
...
- 証拠: `rg -n "ADR-012|revision_hlc|mutation_hlc|base_seq|transactional outbox" docs` でADR、技術仕様、task、STATUS / BACKLOGの参照を確認。`git diff --check` 成功。
- Commit: `7652479`
- 未解決: 現行Rust実装はADR-012未準拠。aggregate削除scope / epochは別ADR裁定待ちで、裁定まではList DEK bundleを削除しない。後続taskでproduction同期テスト、共通client transaction、field clock / placement、typed pull、fuzzy scan + delta full resync、server protocol / RLSを順に実装する。
