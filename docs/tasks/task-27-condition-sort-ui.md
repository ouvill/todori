...
- `todori-private/` 配下を読んだり変更したりしない。private側の課金、収益、法務、監査、公開前ロードマップ詳細をpublic repoへ転記しない。
- `.github/` 配下を変更しない。
...
- `cd todori && sh app/tool/check_hardcoded_strings.sh`
  - 成功。
- `git -C todori diff --check`
  - 成功。
...
- `todori-private/` と `.github/` は読んでおらず、変更していない。
