...
- **2026-07-08ドッグフーディング第4回実施。** フィードバック2件（タスク追加をボトムシート化、Home表示親タスク配下へ期日なしサブタスクも同伴）はtask-54/55へタスク化済み。

## 優先度付きバックログ

| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-54: タスク作成シート | 下部クイック追加バーをタスク作成ボトムシートのトリガーへ変更し、タイトル/Note/List/Due/Add taskをLabモック準拠で実装する | Home改善 / dogfooding follow-up | 出典: ドッグフーディング2026-07-08#4 |
| 2 | task-55: Homeサブツリー同伴表示 | Home各セクションで、表示対象ルートタスクの配下サブツリー全体（期日なしサブタスク含む）を階層ガイド付きで表示する | Home改善 / dogfooding follow-up | 出典: ドッグフーディング2026-07-08#4 |
| 3 | 自然言語日付入力の解析 | クイック追加バー入力中の `tomorrow` / `next Friday` / `明日` 等を日付として解釈する | 将来枠 | 出典: 2026-07-07 Home裁定。task-52ではスコープ外 |
| 4 | task-48予定: Lucideアイコン統一 | 本番アイコンセットを `lucide_icons_flutter` へ統一し、Material Iconsとの混在を解消する。tooltip/semanticsは維持する | 軽量レーン | 出典: 2026-07-06人間裁定のLucide統一。task-43から分離 |
| 5 | Closedセクション見出しの冗長表記整理 | Closedセクション見出しが `"Closed 2 closed"` のように冗長表示される文言を整理する | 軽量レーン | 出典: 親レビュー2026-07-07 |
