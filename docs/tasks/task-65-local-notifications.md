...
- 通知カテゴリ/アクション: category id `todori_reminder_v1`、action id `todori_snooze_1h`。アクション表示は en `+1 hour` / ja `1時間後`。foreground action としてアプリ起動後に `snoozed_until` を更新して再スケジュールする。
...
- スヌーズ: 通知action `todori_snooze_1h` 受信時に同一reminderの `snoozed_until` を現在時刻+1時間へ更新し、同一payloadで再スケジュールする。
