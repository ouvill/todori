...
Taskveilは「Flutter（UI）+ Rust（コアロジック）」構成を採用しており、両者は `flutter_rust_bridge` によるFFIバインディングで接続される（`docs/03_技術仕様書.md` §1.1〜1.3）。現時点のリポジトリでは Flutter アプリ (`app/`) と Rust workspace (`core/`, `cli/`, `mcp-server/`, `server/`) は雛形として別々にコミットされているのみで、両者を繋ぐブリッジは未構築である。
...
`flutter_rust_bridge` v2系を用いて、`app/` から Rust コア（`taskveil-domain` を含む）の関数をDartから呼び出せる最小構成を作る。Linuxデスクトップ環境で動作確認・自動テストができること。
...
2. **ブリッジ用crateの新設**: `app/rust/` ディレクトリに新規Rust crateを作成する。
   - crate名: `taskveil-app-bridge`
   - `app/rust/Cargo.toml` の `[lib]` に `crate-type = ["cdylib", "staticlib"]` を設定する
   - `taskveil-domain.workspace = true` を依存に追加し、リポジトリルート `Cargo.toml` の `[workspace] members` に `"app/rust"` を追加する
...
3. **公開API定義**: `app/rust/src/api.rs`（またはcodegenが要求する規約に沿ったファイル配置。`flutter_rust_bridge_codegen` v2 の標準構成に従うこと）に以下2関数を実装する。
   - `pub fn greet(name: String) -> String` — `format!("Hello {name} from taskveil-core")` を返す。ブリッジそのものが動作することを確認するための最小関数。
   - `pub fn create_draft_task(title: String) -> String` — `taskveil_domain::Task` の下書きインスタンスを1件生成し（`id` は `Uuid::now_v7()`、`status` は `TaskStatus::Todo`、他フィールドは適切なデフォルト値）、`serde_json::to_string` でJSON文字列化して返す。`taskveil-domain` とのリンクが機能していることを実証するための関数。
...
6. **ネイティブライブラリのビルドとロード確認（Linux）**: `cargo build -p taskveil-app-bridge`（または `--release`）でLinux向け `.so` を生成し、Dart側から `DynamicLibrary.open` 等でロードできる場所に配置する手順を確立する（`flutter_rust_bridge` の `RustLib.init()` 相当のAPIを使う場合はそれに従う）。
7. **`app/lib/main.dart` の最小修正**: アプリ起動時（`initState` 等）に `greet("Taskveil")` をRust側に呼び出し、結果をテキストとして画面に表示する。既存のカウンター機能は残しても削ってもよいが、Rust呼び出し結果の表示を追加すること。
...
- `core/storage` / `core/sync` / `core/crypto` とのブリッジ接続（本タスクは `taskveil-domain` の疎通確認のみ）。

## 5. 実装手順（例）

1. `cargo install flutter_rust_bridge_codegen` を実行し、バージョンを確認する（`flutter_rust_bridge_codegen --version`）。
2. `cargo new --lib app/rust --name taskveil-app-bridge` でcrateを作成し、`Cargo.toml` を編集する。
...
- [ ] `cargo clippy --workspace -- -D warnings` が警告ゼロで通過する（`taskveil-app-bridge` がworkspaceメンバーとして含まれること）
...
- `app/rust/` に `taskveil-app-bridge` crateを追加した。
...
- Flutter UIの起動時に `greet("Taskveil")` を呼び、結果を画面に表示するようにした。
...
- Android: `cargo-ndk` またはcargokit等で `taskveil-app-bridge` の `cdylib` をABI別に生成し、Gradleへ組み込む必要がある。
