...
TaskveilのUIは、task-20でThemeData、共通task row、metadata、空状態、dialogなどのUI foundationが整備され、task-21で参考画像 `assets/brand/generated/taskveil-mobile-product.png` の方向性を実アプリへ反映した。

一方で、現時点では「柔らかく・親しみやすく・エレガント」という抽象的な方針はあるものの、今後の画面実装者が参照できる明確な画面スケッチ、モック、デザインルールが不足している。次のゴミ箱画面・復元UI、fractional index/並び替え、通知UIへ進む前に、Taskveilの画面設計をぶらさないための小さなデザイン正本を作る必要がある
...
Taskveilの「柔らかく・親しみやすく・エレガント」を、後続実装者が迷わず参照できるデザイン成果物へ具体化する。
...
1. **デザイン正本用ディレクトリの作成**:
   ...
   - `docs/design/visual-direction.md` を作成し、Taskveilのデザイン方針を文章化する。
2. **画像モックの作成**:
   ...
   - ファイル名は内容が分かる名前にする。例: `taskveil-design-direction-lists.webp`、`taskveil-design-direction-tasks.webp`。
...
3. 「柔らかく・親しみやすく・エレガント」をTaskveil向けに言語化する。例: 静かな日用品、急かさないタスク管理、白い余白、深いグリーン、淡いセージ、控えめな保護シグナル。
...
- [ ] `docs/design/visual-direction.md` に「柔らかく・親しみやすく・エレガント」をTaskveil向けに具体化した説明がある。
...
- [ ] `taskveil-private/` 配下が変更されていない。
- [ ] `git -C taskveil status --short` で意図したドキュメント・画像差分だけが表示されている。
...
- TaskveilはE2EE Todoアプリである。安心感は出すが、未実装のKeychain本実装、アプリロック、同期E2EE、外部監査済み状態を示唆しない。
- Phase 1はローカル専用である。同期、アカウント、課金、Organization、AI、MCP/CLI実機能の画面を先取りしない。
- public repoに残す成果物として、画像・文書とも公開可能な抽象化済み内容にする。
- 既存の `assets/brand/generated/taskveil-mobile-product.png` は参考入力であり、唯一の正解ではない。
...
- 変更しなかったもの（Flutter実装、Rust/FRB/DB/domain、docs/01〜03、taskveil-private）
...
- 読んだファイル:
  ...
  - `docs/tasks/task-21-visual-direction.md`
  - `assets/brand/generated/taskveil-mobile-product.png`
  - `assets/brand/generated/taskveil-desktop-product.png`
  - `assets/brand/generated/taskveil-mascot-kit-refined-no-border.png`
  - `assets/brand/generated/taskveil-mascot-kit-no-sticker-border.png`
  - `app/lib/src/ui/theme.dart`
  - `app/lib/src/ui/task_components.dart`
  - `app/lib/src/ui/states.dart`
  - `app/lib/src/ui/dialogs.dart`
...
- `assets/brand/generated/taskveil-design-direction-tasks.webp`
- `assets/brand/generated/taskveil-design-direction-task-detail.webp`
- `assets/brand/generated/taskveil-design-direction-trash-restore.webp`
- `assets/brand/generated/taskveil-design-direction-empty-dialog.webp`
- `assets/brand/generated/taskveil-design-direction-mobile-focus-tasks.webp`
- `assets/brand/generated/taskveil-design-direction-focus-timer.webp`

補足（2026-07-05）: `assets/brand/generated/taskveil-design-direction-completion-state.webp` は後続の目視判断でTaskveilの方向性と少し異なるため削除した。

### 作成したデザイン文書

- `docs/design/visual-direction.md`
  - 画像モックと既存参考画像/マスコット素材への相対リンクを記載した。
...
- `taskveil-private/` 配下は読んでおらず、変更していない。
- 新規pub依存、UIフレームワーク、icon packageは追加していない。

### 検証結果

- task-22画像8枚をPNGからlossless WebPへ変換し、`file assets/brand/generated/taskveil-design-direction-*.webp ...` でWebPとして存在することを確認した。
- `docs/design/visual-direction.md` からtask-22画像8枚と既存参考画像/マスコット素材への相対リンクを記載した。
- 人間判断により、非採用の既存マスコット画像6枚（`taskveil-mascot-2d-*`、`taskveil-mascot-concept.png`、`taskveil-mascot-kit-identity-board.png`）を削除対象にした。
- `git -C taskveil status --short` で、意図したドキュメント・画像差分のみが残っていることを確認した。
