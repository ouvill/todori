...
| task-35 | 未着手 | [task-35-list-rename-delete.md](./task-35-list-rename-delete.md) | リスト名称変更・削除。domain→storage→bridge→Dart→UIの縦貫通で、M3-01完了条件の残り（作成のみ実装済み）を実装する | task-08〜10の成果物に依存。BACKLOG優先度付きバックログ#1 |
