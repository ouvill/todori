...
Todoriは暗号化・同期・日常のタスク操作を実装しているが、初回起動時は説明なくHomeへ入る。E2EEアプリでは、通常画面へlock表示を常駐させず、初回だけ「ローカル保存」「同期時の暗号化」「ローカル専用データの復旧限界」を平易に伝える必要がある。デザイン面でも、通常画面の情報密度を上げずにTodoriの静かで親しみやすい第一印象を作れる場所はオンボーディングである。
...
- 初回起動時だけ、通常Homeの前にTodoriの価値とプライバシー境界を伝える。
...
- `git diff --check`: 成功
- Rust API、FRB定義/生成物、DB schema、暗号・同期実装、`docs/01〜03`、`.github/`、`todori-private/`は変更していない。
