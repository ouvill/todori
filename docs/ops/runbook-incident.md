...
Todoriの障害分類と初動を定義する。public repo安全のため、実連絡網、顧客対応文面、事業判断、実監視URL、実クレデンシャルは書かない。これらはprivate側または人間管理とする。

## 1. 共通前提

- Todoriはローカルファーストである。サーバーが落ちても、クライアントはローカルDB上の既存データで作成・編集・閲覧を継続できる。
