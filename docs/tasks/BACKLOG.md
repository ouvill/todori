...
| # | タスク | 内容 | 対応マイルストーン | 備考 |
|---|---|---|---|---|
| 1 | task-51: Home画面のセクション再構成 | ルートをHomeへ再定義し、Overdue / Today / Tomorrow / Upcomingセクション、圧縮ヘッダー、行再スタイル、Lists画面Homeリンク改名を行う | M3 polish follow-up | 出典: 2026-07-07 Home裁定 |
| 2 | task-52: 下部常設クイック追加バー | Home/通常リスト画面のAdd task pill/FAB+入力ダイアログを、下部常設入力バーへ置換する | M3 polish follow-up | 出典: 2026-07-07 Home裁定。task-51後 |
| 3 | task-53: タスク行スワイプと軽量モーション | `flutter_slidable` / `flutter_animate` を追加し、leading=完了、trailing=期日変更、軽量モーションを実装する | M3 polish follow-up | 出典: 2026-07-07 Home裁定。依存追加は人間承認済み |
| 4 | 自然言語日付入力の解析 | クイック追加バー入力中の `tomorrow` / `next Friday` / `明日` 等を日付として解釈する | 将来枠 | 出典: 2026-07-07 Home裁定。task-52ではスコープ外 |
...
| 6 | Closedセクション見出しの冗長表記整理 | Closedセクション見出しが `"Closed 2 closed"` のように冗長表示される文言を整理する | 軽量レーン | 出典: 親レビュー2026-07-07 |
