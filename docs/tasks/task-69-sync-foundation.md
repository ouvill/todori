...
Taskveilの同期は「サーバー最新状態方式 + クライアント再push」を採る。サーバーは暗号blobの中身を解釈せず、競合解決に必要な `{fields, field_hlcs}` は暗号境界の内側に置く。pullカーソルはHLCではなくサーバー採番 `seq` であり、HLCはフィールド単位LWWとpush冪等性のためだけに使う。
...
6. `taskveil-crypto` の `encrypt` / `decrypt` を呼び出してblobエンベロープを作る。AAD改ざん、誤DEK、record_id/collection入れ替え、version不一致、サイズ超過をテストする。
...
- `cargo test -p taskveil-sync`: 成功（25 passed、proptest 64ケース）。
