...
- 独立検証では `git -C todori diff dc2661c^ dc2661c --check`、`cargo fmt --all -- --check`、`cargo clippy --workspace -- -D warnings`、`cargo test --workspace`、`cd app && flutter analyze`、`cd app/rust && env CARGO_TARGET_DIR=target cargo build --release`、`cd app && flutter test`、`sh app/tool/check_hardcoded_strings.sh` が成功した。
