...
| 2 | リストのアーカイブ/解除（task-37） | リストのアーカイブ（`lists.archived_at`）/アーカイブ解除を実装する。アーカイブ済みリストは通常一覧から分離表示（アーカイブ済みセクション等）し、データ・完了履歴は完全保全する（F-09改訂準拠） | M3-01相当 | 出典: 2026-07-07人間裁定（削除モデル）。docs/02 F-09改訂準拠。task-36（DBスキーママイグレーション機構）完了後に着手 |
| 3 | ゴミ箱廃止と恒久削除への移行（task-38） | trash UI/route/API（task-23実装分）を撤去し、タスク・リストの削除を物理削除＋不可逆警告の追加確認へ移行する。削除Undoは廃止する（完了/編集Undoは維持）。`tasks.deleted_at`カラムおよびundo履歴の削除系エントリの整理はマイグレーション経由で行う | M3-01 / M3-05相当 | 出典: 2026-07-07人間裁定（削除モデル）。docs/05 ADR-009 / docs/02 F-07改訂準拠。task-37（リストアーカイブ）の直後に着手 |
| 4 | タスク行のdot/チェック整列修正 | priority dotとチェックをタイトル1行目とセンター整列させる（行全体センターではない） | 軽量レーン | 出典: 親レビュー2026-07-06（`docs/design/ui-spec.md` セクション5「既知の逸脱」参照） |
